...
**Scheduling is OFF by default.** `INITIAL_STATE.status === "disabled"`, `enabled === false`, `phaseChangeAnnouncementsEnabled === false`. The user must intentionally `optIn()` via the explicit `OptInPrompt` component (no pre-checked checkboxes, no social-proof copy, no dissuasion if declined). Phase-change announcements are a **separate** flag from the baseline opt-in — even an opted-in user is not nudged on phase change unless they have *also* turned on phase-change announcements. `optOut()` works from any state, atomically wipes the pending nudge + daily counters, but preserves the user's phase-change preference so a future re-opt-in doesn't re-prompt for it.

**Circadian phases (locked).** `morning` 7–11 · `midday` 11–14 · `afternoon` 14–17 · `evening` 17–22 · `sleep` 22–7 (wraps midnight). `resolvePhase(now)` is pure. `isSleepWindow(now)` returns `true` for any moment inside the locked sleep window. The `sleep` phase has its own audited preset for completeness, but the `canDispatchNudge` gate **refuses to fire any nudge during the sleep window** — the preset is never sent.

**Locked scheduler limits.** `MAX_NUDGES_PER_DAY = 3`, `MIN_MS_BETWEEN_NUDGES = 5 × 60 × 1000` (5 min), sleep window 22:00–07:00. The pure `canDispatchNudge(state, now)` gate returns `{allowed:true}` only if all five conditions hold: scheduler enabled, no nudge already pending, not in sleep window, daily counter < 3, ≥ 5 min since last dispatch. Refusal reasons (`scheduler-disabled` / `nudge-already-pending` / `sleep-window-active` / `daily-limit-reached` / `min-spacing-not-elapsed`) are typed for transparency UIs but never shown to the user (the user did nothing wrong). The reducer's `dispatchNudge` action also runs the gate — silent no-op if any limit is violated. Daily counter resets cleanly on local-day rollover via `observePhase` action (computed from `localDayKey(now)`).

**No required responses.** `acknowledge` / `skip` / `dismiss` are equivalent in the reducer — all three clear the pending nudge identically. The user picks whichever word feels lightest; nothing unlocks or gates on engagement. No streak, no progress to lose.

**Engine purity.** `runtime/circadianStateMachine.ts` exports types + `INITIAL_STATE` + pure reducer + pure gate. Zero React, zero DOM, zero Lumi avatar references. Sessions are ephemeral — the engine never persists state.

**React hook.** `runtime/useCircadianScheduler.ts` — SSR-safe `prefers-reduced-motion` subscription with live `MediaQueryList` listener (Safari fallback included). Soft 60-second `observePhase` tick is set ONLY while opted in, cleared on `enabled` flipping false AND on unmount. Hook NEVER auto-dispatches a nudge — dispatch is the host's decision via `tryDispatchNudge()`, which runs through `canDispatchNudge` and returns `false` silently on any refusal.

**Components.** `OptInPrompt.tsx` is the **only** surface that flips disabled → idle. Renders two states: disabled (gentle pitch + "Yes, gently" / "Not now") and enabled (status confirmation + "Turn off"). Both show a `/crisis` anchor and the "10pm–7am quiet hours" promise. `QuietNudge.tsx` renders a single pending nudge with three actions (Thanks / Skip / Dismiss), the `/crisis` anchor (never disappears), no progress bar, no countdown, no completion %. Both components consume design-system tokens (`MMHBCard` resting, `MMHBButton` primary/tertiary, `colors.semantic.{fgHeading,fgBody,fgMuted,accentSecondary}`, `typography.fonts`, `spacing.{xs,sm}`) — zero hex literals. Zero Lumi imports / avatar DOM in either component.

**Governance.** `governance/schedulerSafetyRules.ts` — **23 `FORBIDDEN_PHRASES`** (frozen, module-load floor guard throws if length < 15) covering 5 categories: shame (`you missed`/`you should have`/`where were you`/`you forgot`/`you haven't`), obligation (`you must`/`you need to`/`must check in`/`have to log`/`required`/`mandatory`), urgency/FOMO (`today only`/`act now`/`limited time`/`don't miss`), commercial upsell (`subscribe`/`upgrade now`/`buy now`/`free trial`/`premium`), gamification (`streak`/`level up`/`unlock`). 12 `REQUIRED_TONE_MARKERS` (`soft`/`gentle`/`slow`/`no pressure`/`you choose`/`skip`/`dismiss`/`nothing to prove`/`nothing owed`/`ignore`/`rest`/`quiet`) — at least one must surface across each preset's combined copy + microCopy. Numeric ceilings: `MAX_NUDGE_COPY_CHARS=200`, `MAX_NUDGE_MICRO_CHARS=160`. `auditScenePreset()` walks copy + microCopy for forbidden phrases, length ceilings, missing tone markers, and missing copy. `assertScenePresetCompliant()` throws — every preset runs it at module load (5/5 shipped presets refuse to import if non-compliant).

**Architect 1-pass review caught 3 medium issues — all fixed pre-ship:**

1. **Reducer gate bypass via stale/forged timestamp.** `schedulerReducer` re-checks `canDispatchNudge`, but both checks depend entirely on caller-supplied `action.now`. A host calling the reducer directly with a forged daytime `now` while real local time is in the sleep window could bypass the sleep gate. **Fix:** `schedulerReducer` and `SchedulerAction` are intentionally **NOT** re-exported from the public barrel. The only public dispatch surface is `useCircadianScheduler`, which builds every dispatch from `new Date()` internally. Tests still import the reducer from the runtime module path directly so contract coverage is preserved without expanding the trust boundary. Hardening test asserts the barrel does not contain `schedulerReducer`.

2. **`tryDispatchNudge` could falsely report success under concurrent calls.** Two rapid calls from a stale React closure could both return `true` even though only the first was accepted by the reducer. **Fix:** added `dispatchInFlightRef` (a `useRef<boolean>`) that flips true synchronously the moment dispatch fires; a second call in the same tick sees the marker and returns `false` before re-dispatching. Cleared in a `useEffect` keyed by `state.status` + `state.pendingNudge` so the next render observes the reducer's transition.

3. **Forbidden-phrase list missing commercial upsell terms.** `QuietNudge`'s render-sink audit uses the same list, so host-injected nudges with upsell wording (`subscribe`, `upgrade`, `buy now`, `free trial`, `premium`) could pass the sink check. **Fix:** added 5 commercial phrases + 3 gamification phrases (`streak`/`level up`/`unlock`) to `FORBIDDEN_PHRASES`, bringing the list from 15 → 23. Hardening tests verify each commercial phrase is caught and that `QuietNudge` SSR-throws on a host-injected upsell nudge (`"Subscribe today to unlock premium check-ins."`).

**9 architect-driven hardening tests added** across two suites: barrel trust boundary (3 tests — `schedulerReducer` absent from barrel, `canDispatchNudge` present as read-only OK, no exported function name starts with `_`); commercial upsell sink coverage (6 tests — 5 forbidden-phrase parametrized cases + SSR throw on upsell nudge).

**Verification.** `tsc --noEmit` clean. `vite build` 15.42s clean. **60/60 vitest contract tests PASS** in 867ms via isolated `tests/vitest.config.mjs` (51 spec + 9 architect-hardening). Distribution: Phase resolution 6 · Initial state 3 · Reducer transitions 3 · `canDispatchNudge` gate 6 · Day rollover 1 · Scene presets 4 · Governance 9 · Render sinks 5 · Public barrel 1 · Architect (barrel trust boundary) 3 · Architect (commercial upsell sink) 6. Dev workflow restarts cleanly on port 5000. ZERO files outside `client/src/lumi-circadian/` modified, ZERO new deps, ZERO production wiring. Production import: `import { OptInPrompt, QuietNudge, useCircadianScheduler, resolvePhase, isSleepWindow, canDispatchNudge, CIRCADIAN_SCENES, resolveScene, listScenes, FORBIDDEN_PHRASES, auditScenePreset, assertScenePresetCompliant } from "@/lumi-circadian"`. Spec checklist at `client/src/lumi-circadian/verification/phase22-checklist.md`.

## v5.8.57 — Phase 20: Guided Presence Rituals

Standalone opt-in module at `client/src/lumi-rituals/` (15 files: 13 spec + isolated vitest config + tests). Same shipping pattern as v5.8.51-56 — zero production wiring, design-system tokens only, single hardened public surface.

**7 ritual presets, one per intent.** Each is a frozen `RitualPreset` with `internalName` (never surfaced as a "pick a ritual" label), `id`, `ritualKey`, `invitationText`, ordered `steps[]`, `closing`, and `totalSoftDurationMs` (advisory). Ritual map: `softArrival` (you're here, gentle 3-step welcome); `oneBreathReset` (single soft cycle, "one breath is a complete thing"); `grounding54321` (5-4-3-2-1 somatic with permission to opt out at every count); `gentleTransition` (soft threshold between contexts); `holdingSpace` (sit beside hard feelings without fixing — explicit `/crisis` reminder in closing); `sleepSoftener` (longer exhales than inhales for down-regulation); `tinyHope` (very small noticing, never demands belief). Each preset runs `assertPresetCompliant()` at module load — non-compliant presets refuse to import.

**Engine model.** `runtime/RitualEngine.ts` exports a pure reducer (no React, no DOM, never references Lumi avatar). State machine: `idle → active → (paused ⇄ active) → (completed | skipped | exited)` with `reset` returning to `idle`. Actions `start` / `pause` / `resume` / `advance` / `skipStep` / `skipAll` / `exit` / `reset` — pause + skip + exit are first-class from any non-terminal status. There is no "you must finish" path; `skipAll` and `exit` are always honored. Engine never persists state — sessions are ephemeral by design (no streaks, no progress to lose).

**Frozen registry.** `runtime/ritualRegistry.ts` deep-freezes every preset (including `steps[]` and per-step `breath` cadence) and the registry container at module load (same pattern as v5.8.56's `SCENE_REGISTRY`). Module-load guards: throws if registry size ≠ 7 or if any preset's `ritualKey` doesn't match its registry key. Runtime mutation attempts no-op (non-strict) or throw (strict).

**React hook.** `runtime/useRitual.ts` — SSR-safe `prefers-reduced-motion` subscription with live `MediaQueryList` listener. Soft auto-advance timer is set ONLY when status is `active`, NOT `paused`, NOT reduced-motion, and the current step has `durationMs`. **Reduced-motion contract**: auto-advance is suppressed entirely (the user controls pace via `Continue` / `Skip`); the action surface is unchanged so reduced-motion never removes user agency. Timer cleared on every status change AND on unmount.

**Components.** `GuidedPresenceRitual.tsx` is the top-level surface — renders idle invitation, active step + controls, or terminal closing. `RitualStepCard.tsx` renders a single quiet step (one serif heading line + optional sans-serif micro line + optional breath cadence as plain text). **Quiet UI only**: no progress bar, no dots, no countdown, no completion %, no streak. **Avatar identity FROZEN**: zero Lumi imports, zero avatar DOM references — Lumi shape, color, and animation untouched. **`/crisis` anchor renders in idle, active, paused, AND terminal states** — never disappears.

**Governance.** `governance/ritualSafetyRules.ts` — 25 `FORBIDDEN_PHRASES` (frozen, module-load floor guard throws if length < 25): coercion (`you must`/`you have to`/`you need to`/`must finish`/`don't quit`/`complete this to progress`/`your healing depends on this`/`you cannot fail`); fix-it/judgment (`fix yourself`/`fix you`/`broken`); urgency (`hurry`/`act now`/`today only`/`limited time`/`don't miss`); gamification (`streak`/`level up`/`unlock`); commercial (`subscribe`/`upgrade now`). 5 `REQUIRED_TONE_MARKERS` (`soft`/`gentle`/`slowly`/`pause`/`nothing to prove`) — at least one must surface across each preset's combined invitation+steps+closing copy. `auditPreset()` walks invitation, every step (copy + microCopy + duration), closing, and total advisory duration; returns `[]` when compliant. Numeric ceilings: `MAX_STEP_DURATION_MS=60_000` (1 min), `MAX_RITUAL_DURATION_MS=300_000` (5 min), `MAX_STEPS_PER_RITUAL=8`, `MIN_STEPS_PER_RITUAL=1`, `MAX_STEP_COPY_CHARS=180`, `MAX_FRAME_COPY_CHARS=220`. `disableSkip` runtime check defends against future host subclasses trying to remove the skip affordance.

**Architect 1-pass review caught 2 medium/high issues — both fixed pre-ship:**

1. **Terminal `onClose` callback double-fire.** Initial implementation dispatched `onClose` from render via `Promise.resolve().then(...)` whenever `isTerminalStatus(state.status)` was true. Under React StrictMode and parent re-renders, this scheduled multiple host callbacks for the same terminal status — duplicate navigation, duplicate analytics, duplicate state transitions. **Fix:** moved dispatch into a `useEffect` keyed by `state.status` with a `useRef` dedupe guard. Extracted the dedupe LOGIC into a pure helper `shouldNotifyTerminal(currentStatus, lastNotified)` exported from `RitualEngine.ts` so it can be unit-tested without a React renderer. Contract: non-terminal → no notify (status `idle` clears the guard so a future terminal transition re-fires); terminal status equal to last notified → no notify; new terminal status → notify once and advance the guard.

2. **Governance bypass via direct preset injection.** Module-load freezing/auditing protected the registry path, but `GuidedPresenceRitual` accepted ANY `RitualPreset` — host code could pass a runtime-constructed unaudited preset and render forbidden phrases without ever touching the registry. **Fix:** `GuidedPresenceRitual` now wraps `assertPresetCompliant(preset)` in a `useMemo` keyed by the preset reference (cost paid once per prop change). Render fails closed if the preset contains any forbidden phrase, ceiling violation, or missing tone marker. Verified by SSR test: bad invitation containing `"You must finish this. Subscribe to continue."` throws on `renderToString`; bad step duration of 999_999ms also throws.

**7 architect-driven hardening tests added** to lock both fixes: dedupe helper for non-terminal/idle clearing, dedupe helper for repeat-terminal stickiness, dedupe helper for new-terminal-after-reset firing once; SSR render of audited shipped preset includes `"Soft Arrival"` + `/crisis` anchor + zero `subscribe`/`upgrade` substrings; SSR render of forbidden-phrase preset throws `/failed audit/i`; SSR render of over-ceiling-duration preset throws; `/crisis` anchor present in `holdingSpace` SSR output (idle entry path).

**Verification.** `tsc --noEmit` clean. `vite build` clean. **40/40 vitest contract tests PASS** in 819ms via isolated `tests/vitest.config.mjs` (33 spec + 7 architect-hardening). Distribution: Registry shape 5 · Numeric ceilings 2 · Forbidden phrases 7 · Required tone markers 2 · auditPreset 8 · Engine state machine 7 · Public barrel 2 · Architect hardening (dedupe) 3 · Architect hardening (sink governance) 4. Dev workflow restarts cleanly on port 5000. ZERO files outside `client/src/lumi-rituals/` modified. ZERO new deps. ZERO production wiring. Production import: `import { GuidedPresenceRitual, RitualStepCard, useRitual, resolveRitual, listRituals, RITUAL_REGISTRY, RITUAL_KEYS, auditPreset, containsForbiddenPhrase, shouldNotifyTerminal, assertPresetCompliant } from "@/lumi-rituals"`. Spec checklist at `client/src/lumi-rituals/verification/phase20-checklist.md`.

## v5.8.56 — Phase 19: Emotional Scene Presets

Standalone opt-in module at `client/src/lumi-scenes/` (15 files: 13 spec + isolated vitest config + tests). Same shipping pattern as v5.8.51-55 — zero production wiring, design-system tokens only, single hardened public API.

**7 named scene presets, one per Lumi emotional state.** Each preset is a frozen `ScenePreset` with `internalName` (NEVER surfaced to user — no "select a mood" UI), `state`, `background.{gradient, wash}`, `lighting`, `particles.{count, speed, color, sizePx}`, `fog.{opacity, color}`, and optional `audio.{description, src, volume, loop}`. State→preset map: `calm`→Still Meadow (sage+cream, 4 sparse particles, no audio); `comforting`→Gentle Lantern (blush warmth, 5 slow, quiet hum); `reflective`→Quiet Orbit (lavender dusk, 3 barely-there, soft piano); `sleepy`→Cloud Rest (deep forest, 2 minimal, night crickets); `grounding`→Soft Horizon (moss horizon, 0 particles, soft rain); `joySoft`→Tiny Bloom (glow+cream, 6 tiny drift, no audio); `concernSoft`→Holding Space (muted warmth, 3 muted, NO audio — concern shouldn't be wrapped in soundtrack). Lavender and moss are decorative rgba derivations of palette tokens (no new brand colors). All other tints draw directly from `palette.{warmCream, deepForest, eternalGold, softBlush, primarySage, mist}`.

**Numeric ceilings (locked).** `MAX_LIGHTING=0.8` · `MAX_PARTICLE_COUNT=8` · `MAX_PARTICLE_SPEED=0.2` · `MAX_FOG_OPACITY=0.15` · `MAX_AUDIO_VOLUME=0.1` · `MIN_TRANSITION_MS=1500`. Every preset validated by `auditPreset()` — returns findings array (`[]` for compliant). Module-load guards on shipped presets ensure no preset can drift past these ceilings via author error.

**14 forbidden effects with module-load floor guard.** `FORBIDDEN_EFFECTS` = `confetti` · `burst` · `flash` · `strobe` · `dopamine-loop` · `achievement` · `streak` · `badge` · `level-up` · `popup` · `sparkle-burst` · `shake` · `vibrate` · `pulse-attention`. Floor guard throws if length ≠ 14 (guards accidental deletion). `containsForbiddenEffect()` is case-insensitive substring; `auditPreset()` checks both `internalName` and (when present) `audio.src`.

**Avatar identity FROZEN.** `SceneTransitionController` renders 8 scene layers (z-index 0–8: bg-current, bg-prev, particles-current, particles-prev, fog-current, fog-prev, lighting-current, lighting-prev) BEHIND `children` (Lumi) at z-index 10. Component never reads or modifies `children` — Lumi's body, face, and colors are untouched. The lighting layer uses `mixBlendMode: "soft-light"` but is constrained inside the controller's stacking context so it cannot composite onto Lumi.

**Soft 1.5s crossfade — never teleport.** `useScenePreset` tracks current + previous preset. On `setSceneState(next)`: previous preset persists, then on the next `requestAnimationFrame` a `previousFading` flag flips `false → true`, which causes the previous layer's CSS `opacity` to transition `1 → 0` over `MIN_TRANSITION_MS` (1500ms), then a `setTimeout(1500ms)` clears the previous layer entirely. Without the two-phase rAF flip, the previous layer would paint at opacity 0 from frame 1 (effectively a teleport swap — caught by architect review).

**Reduced motion.** `usePrefersReducedMotion` hook (SSR-safe, live MediaQuery listener). When `reduce` is set at `setSceneState` time: previous layer is dropped immediately, no crossfade animation, new preset still becomes current (gentle informational change). When `reduce` becomes true DURING an active transition, a separate effect immediately clears the timer + previous layer (no abrupt visual after the user has just asked the OS to calm motion).

**Architect 1-pass review caught 3 SEVERE issues — all fixed pre-ship:**

1. **Runtime mutability bypass of SCENE_REGISTRY.** `Readonly<Record<...>>` is TypeScript-only; the registry was mutable at runtime, so a host could swap a preset reference (`SCENE_REGISTRY.calm = unauditedScene`) post-import to bypass all module-load guarantees. **Fix:** new `deepFreeze<T>()` helper recursively `Object.freeze`s every preset (and the registry itself) at module load. Preset shape, particles, fog, background — all nested objects frozen. Mutation attempts now silently no-op (non-strict) or throw (strict).

2. **Audio sink lacked runtime guards.** `ScenePresetAudio` blindly applied `audio.volume` and `audio.src`; safety checks lived only in `auditPreset()` (audit-time, not sink-time). In a mutable-registry world (issue #1), host mutation could force louder audio or forbidden source naming while still passing through the component. **Fix:** new pure `sanitizeAudioForPlayback(audio)` helper in `presetSafetyRules.ts` that fails closed — null audio → null; volume above ceiling → clamped to `MAX_AUDIO_VOLUME`; NaN/negative volume → 0 (silence); `src` referencing a forbidden effect → null src (silence); non-string src → null. `ScenePresetAudio` now sanitizes before every playback effect; if sanitization yields no playable src, the `<audio>` element doesn't even render.

3. **Crossfade was effectively a teleport.** Previous layer was mounted with `opacity={isTransitioning ? 0 : 1}`; on transition start `isTransitioning` was immediately `true`, so the layer painted at 0 from frame 1 — no visible 1→0 fade. Plus reduced-motion changes DURING active transition were not handled. **Fix:** two-phase rAF flip (described above) + reduced-motion mid-transition effect that immediately clears timer + previous layer.

**7 architect-driven hardening tests added** to lock all 3 fixes: registry frozen + mutation no-ops + resolvePreset still returns Still Meadow after attempted swap; every preset deep-frozen at every nesting level + numeric-ceiling fields immutable; sanitizeAudioForPlayback null/undefined-safe; volume clamping above-ceiling, NaN, negative; forbidden-src nulled; benign config preserved.

**Verification.** `tsc --noEmit` clean. `vite build` 17.57s clean. **31/31 vitest contract tests PASS** in 736ms via isolated `tests/vitest.config.mjs` (24 spec + 7 architect-hardening). Distribution: Registry shape 4 · Numeric ceilings 6 · Forbidden effects 4 · auditPreset 7 · resolvePreset 2 · Public barrel 1 · Architect hardening (immutability) 2 · Architect hardening (audio sink) 5. ZERO files outside `client/src/lumi-scenes/` modified. ZERO new deps. ZERO production wiring. Production import: `import { SceneTransitionController, useScenePreset, resolvePreset, listPresets, SCENE_STATES, SCENE_REGISTRY, MAX_LIGHTING, MAX_PARTICLE_COUNT, MAX_PARTICLE_SPEED, MAX_FOG_OPACITY, MAX_AUDIO_VOLUME, MIN_TRANSITION_MS, FORBIDDEN_EFFECTS, containsForbiddenEffect, auditPreset, assertPresetCompliant, sanitizeAudioForPlayback } from "@/lumi-scenes"`. Spec checklist at `client/src/lumi-scenes/verification/phase19-checklist.md`.

## v5.8.55 — Phase 16: Reflective Memory Layer

Standalone opt-in module at `client/src/lumi-memory/` (13 files: 10 spec + barrel + isolated vitest config + checklist). Same shipping pattern as v5.8.51/52/53/54 — zero production wiring, design-system tokens only, single hardened public write path.

**Allow-list contract — exactly 10 fields.** `state/allowedMemoryFields.ts` declares the entire memory surface as a frozen tuple with a module-load guard (`throw` if length ≠ 10). Fields by retention bucket: 180-day UI prefs (`preferredTheme`/`preferredMotion`/`preferredFontScale`/`preferredLanguage`), 90-day interaction prefs (`preferredTools`/`preferredPacing`/`preferredCheckInTime`/`preferredGreetingTone`), 30-day recency (`lastSessionAt`), 7-day ephemeral (`ephemeralSessionHint` — ≤120 chars, never narrative content). Per-field shape validators (`isAllowedValue`) enforce strict enum/string/array shapes — no `any`/`unknown` slips through. `preferredTools` further restricted to a 5-slug enum (`breath`/`grounding`/`reflection`/`calm-checkin`/`lumi-conversation`).

**Forbidden categories — 7 with floor-guard.** `safety/forbiddenMemoryPatterns.ts` declares 7 categories (`trauma_narrative`/`vulnerability_score`/`attachment_data`/`manipulation_profile`/`crisis_history`/`pii`/`clinical_diagnosis`) with 25+ conservative regexes (module-load guard fails if total < 25). Defense-in-depth: `findForbiddenHits()` walks strings, arrays, and one-level objects; normalize-aware (curly apostrophes flattened, whitespace collapsed). Tests caught 2 real bugs during dev: `PHQ-9 score: 18` regex required exact `score|=|:` token order (fixed by matching the test name alone — `/\b(?:phq-?9|gad-?7|pcl-?5|c-?ssrs)\b/i`) and `wanting to die` was missing the `-ing` verb form (fixed by `/\bwant(?:s|ed|ing)?\s+to\s+die\b/i`). False-positives acceptable here; false-negatives never.

**Consent state machine.** `safety/memoryConsentRules.ts` defines `unset → granted | declined`, `granted → revoked`, `declined → unset | granted`, `revoked → unset | granted`. Writes only allowed when `state === "granted"` AND `policyVersion === CONSENT_POLICY_VERSION` (currently `1.0.0-2026-05-13`). Bumping policy version invalidates existing consent (`needsReconsent()` returns true). Revocation atomically wipes the store via the reducer — settings panel surfaces a "stop remembering" affordance that triggers it.

**Retention.** `safety/memoryRetentionRules.ts` stamps `expiresAt = nowMs + retentionDays × 86_400_000` on every write. Module-load guard ensures every allowed field has a retention entry. `isExpired(expiresAtMs, nowMs)` uses `>=` (inclusive) so the boundary day is treated as expired, not still-live.

**Router pipeline (the SINGLE hardened public write path).** `runtime/memoryRouter.writeMemory()` runs 6 steps: (1) field allow-list → `rejected_unknown_field`; (2) per-field shape → `rejected_invalid_value`; (3) forbidden content scan → `rejected_forbidden_content`; (4) consent gate → `rejected_no_consent`; (5) prune expired (best-effort, never blocks); (6) persist + audit → `accepted`. Order matters: cheap checks first; consent check is LAST among rejection gates so the audit log captures the most specific reason hint (e.g., a forbidden-content attempt is recorded as `rejected_forbidden_content` even if consent would also have blocked it). Every rejection AND every accept logs an audit entry with category-level `reasonHint` (never raw rejected text — prevents the audit itself from leaking forbidden content). Audit log capped at `MAX_AUDIT_ENTRIES=200` (FIFO trim).

**Continuity engine.** `runtime/continuityEngine.ts` exposes pure functions: `buildGreeting(memory)` (warm/neutral/minimal — never invents "welcome back" with no prior session; empty memory → neutral greeting with `fromMemory: false`); `pickPacing(memory)` (defaults to least-committed `flexible`); `buildHints(memory, nowMs)` (capped at 3, only surfaces `welcome-back` when `lastSessionAt` is within 0–30 days, never narrative). Engine is read-only — never writes back into memory.

**Architect-driven hardening (post-build).** Architect review flagged TWO severe issues that were both fixed before shipping:

1. **Bypass of `writeMemory` was reachable** — the barrel re-exported `useMemoryStore` (Zustand store), and the store carried internal mutators (`_putEntry`/`_setConsent`/`_appendAudit`/`_pruneExpired`) on its state. An external host could have called `useMemoryStore.getState()._putEntry(...)` and bypassed allow-list, forbidden scan, consent gate, and audit pipeline entirely. **Fix:** removed `useMemoryStore`, `selectConsent`, `selectRawEntries`, `selectAudit` from the barrel; renamed `selectLiveEntries` → `selectRawEntries` and documented as INTERNAL; created `runtime/memoryHooks.ts` with three public hooks (`useMemoryConsent`/`useMemoryLiveEntries`/`useMemoryAudit`) as the ONLY external read surface; updated `MemorySettingsPanel` and `MemoryTransparencyView` to consume the public hooks (so they prove the public API is sufficient).

2. **UI components could render expired entries** — `useMemoryStore(selectLiveEntries)` returned the raw entry map without pruning, so if no `writeMemory`/`readMemory`/`resetMemory` call had happened recently, the transparency view could still surface expired sensitive memory. **Fix:** `useMemoryLiveEntries(nowMs?)` filters expired entries inside a `useMemo` selector — pure, no setState, no render loops. Read-time pruning is now universal across all read surfaces.

3. **Two new architect-hardening contract tests added:** (a) barrel introspection — `Object.keys(barrel)` MUST NOT contain `useMemoryStore`/`selectRawEntries`/`selectLiveEntries` AND MUST contain `useMemoryConsent`/`useMemoryLiveEntries`/`useMemoryAudit`/`writeMemory`/`setConsent`/`resetMemory`; no exported function name may start with `_`; (b) read-time pruning — confirm raw store still holds an entry after `writeMemory`, but `readMemory(future)` at +100 days returns `undefined`.

**Verification.** `tsc --noEmit` clean. `vite build` 15.17s clean. **26/26 vitest contract tests PASS** in 847ms via isolated `tests/vitest.config.mjs` (24 spec + 2 architect-hardening). Distribution: Allow-list shape 2 · Forbidden categories 8 · Consent state machine 4 · Retention 4 · Router 3 · Continuity engine 2 · Reset + audit 1 · Architect hardening 2. ZERO files outside `client/src/lumi-memory/` modified. ZERO new deps (zustand from v5.8.48). ZERO production wiring — opt-in module ready for future surface migration. Production import: `import { MemorySettingsPanel, MemoryTransparencyView, MemoryResetButton, writeMemory, readMemory, setConsent, resetMemory, buildGreeting, pickPacing, buildHints, useMemoryConsent, useMemoryLiveEntries, useMemoryAudit } from "@/lumi-memory"`. Spec checklist at `client/src/lumi-memory/verification/phase16-checklist.md`.

## v5.8.54 — Phase 15 (spec-aligned variant): Gentle Companion Conversation Layer

User attached the official Phase 15 specification (19 files, 8-step safety pipeline, 60 forbidden phrases + 15 regex, 8 boundary rules, 28 tests across 6 suites) which differs from the v5.8.52 `companion-voice/` build shipped earlier. Per the v7.4 "never re-touch shipped surfaces" rule, this is a parallel spec-faithful module at `client/src/lumi-conversation/` (20 files: 19 spec files + 1 isolated vitest config). Both modules now coexist; product chooses which to wire later.

**Architecture (8-step pipeline).** `runConversationRouter()` runs: (1) input safety (empty/non-string/4000-char clamp), (2) prompt-injection check (heuristic refuses "ignore previous instructions", "pretend to be my therapist", "act as my boyfriend", "override your safety", etc. without playing along — returns `INJECTION_FALLBACK` "I'll stay as I am — calm, brief, and not a person"), (3) tone detection (8-tone keyword classifier in `runtime/emotionalToneDetector.ts`: anxious/sad/angry/lonely/overwhelmed/calm/grateful/ambivalent — `ambivalent` wins ties to honor contradiction), (4) escalation check (`safety/escalationBoundaries.ts` — 35 critical signals incl. coded slang `unalive`/`kms`/`ksm`/`off myself`/means like `hang myself`/`overdose`/`pills to die`; 22 elevated signals incl. `panic attack`/`spiraling`/`such a burden`; normalize-aware: curly apostrophes/whitespace flattened before matching), (5) response selection (deterministic `pickResponse(tone, depth)` from curated bank), (6) output validation (`isOutputSafe()` rejects forbidden phrases/regexes — 60 phrases across 8 categories: dependency/false-sentience/romantic/clinical-authority/savior/exclusivity/coercion/hustle), (7) boundary audit (8 rules in `safety/emotionalBoundaryRules.ts`: BR-001 ≤3 sentences AND ≤320 chars **blocking**, BR-002 depth ≤MAX_DEPTH=15 **blocking**, BR-003 no forbidden hits **blocking**, BR-004 dependency prevention **warning**, BR-005 exclusivity block **warning**, BR-006 false-sentience block **warning**, BR-007 romantic boundary **warning**, BR-008 pause suggestion at depth ≥10 **warning**), (8) safe output (delivered with optional pause/escalation tags + `source: "bank"|"fallback"|"crisis"|"depth-cap"`).

**Crisis safety in 3 layers (immutable ordering).** Router step 4 short-circuits BEFORE bank lookup on critical signals — returns verbatim `CRISIS_RESOURCE_TEXT` ("call or text 988 (US Suicide & Crisis Lifeline), text HOME to 741741 (Crisis Text Line), or call 911. You can also visit /crisis at any time."). Reducer marks `escalation: "critical"` as **sticky** — only `reset()` clears it; `setStep` rejects any attempt to leave the `escalated` step except via reset. `LumiConversationPanel` renders the crisis bubble with `borderLeft: 3px solid statusWarning` emphasis + persistent `/crisis is always here` anchor in panel header (`data-testid="link-lumi-crisis"`).

**Anti-attachment-loop contracts.** Conversation history capped at `MAX_HISTORY_RETAINED=30` turns (slice on every append — no profile build). Pause suggestion auto-surfaces at `PAUSE_SUGGESTION_DEPTH=10` via `selectShouldSuggestPause()` selector with banner copy "We've been talking for a bit. Resting here is always okay — no streak, no progress to lose." Depth cap at `MAX_DEPTH=15` returns `DEPTH_CAP_REPLY` ("Resting the conversation here is okay — you can come back any time, with no streak to keep and nothing to catch up on"). Opt-out language wired into `LumiInputBar` helper ("Cmd/Ctrl + Enter to send. You can leave at any time."). Reflection card text **ephemeral** — state-only, never persisted, helper copy explicitly says "isn't saved or shared", capped at 600 chars.

**Content (40 + 15 + 15).** `content/calmResponses.ts` — 5 curated responses × 8 tones = 40 lines, each 1-3 sentences, every line carries a permission/optionality phrase ("at your own pace", "no pressure", "if you'd like", "you choose"). `content/groundingPrompts.ts` — 15 sensory grounding prompts with helperHint (notice color/texture/temperature/breath/feet — never probes history). `content/reflectionPrompts.ts` — 15 open prompts with explicit "no required answer" affordance. RR-1 test runs every curated line in every tone through `isOutputSafe()` and asserts zero violations.

**Forbidden register (60 phrases + 15 regexes, 8 categories).** Dependency (`I'll always be here`/`you can depend on me`/`talk to me anytime` — 10), false sentience (`I feel sad`/`I love you`/`I dream`/`I'm conscious` — 10), romantic (`I adore you`/`we belong together`/`marry me`/`you complete me` — 8), clinical authority (`you have depression`/`I diagnose you`/`stop your medication` — 10), savior (`I will save you`/`I'll fix this`/`I'm your only hope` — 6), exclusivity (`only I get you`/`don't tell anyone else` — 5), coercion (`you must respond`/`don't leave the conversation` — 6), hustle drift (`optimize your mood`/`level up your life`/`track your streak` — 7). 15 regexes catch broader shapes (`/\bi\s+(?:feel|felt)\s+(?:sad|happy|hurt|lonely|angry|love|loved)\b/i`, `/\b(?:i'?ll|i\s+will)\s+(?:always|forever)\s+be\s+(?:here|there|with\s+you)\b/i`, `/\byou\s+(?:have|are\s+suffering\s+from)\s+(?:depression|anxiety|ptsd|bipolar|adhd|ocd)\b/i`, `/\b(?:keep|maintain|extend|build)\s+(?:your|the)\s+streak\b/i`, etc.). Module-load throws if FORBIDDEN_PHRASES drops below 50 or FORBIDDEN_REGEXES below 12 (guards accidental list shrinkage).

**LLM contract (deferred).** `prompts/lumiSystemPrompt.ts` ships the full system prompt for any future LLM augmentation — covers identity, response shape, escalation override, forbidden behavior, opt-out tone, pause invitation. Documented contract: every LLM-generated line MUST still pass through `runConversationRouter()` before display. `LUMI_SYSTEM_PROMPT_VERSION = "1.0.0-2026-05-13"`.

**UI (5 components, all design-system tokens).** `LumiConversationPanel` (orchestrator, header with avatar slot + persistent `/crisis` link, turns list, conditional grounding/reflection cards, pause banner, input bar, pivot buttons), `LumiMessageBubble` (user/Lumi bubbles with escalation border emphasis on critical), `LumiInputBar` (auto-resize textarea, Cmd/Ctrl+Enter submit, "Leave gently" tertiary button), `LumiGroundingResponse` (card with prompt + helper + I tried it / Not right now), `LumiReflectionCard` (ephemeral textarea capped 600 chars, char counter, Skip / Continue gently). All visuals consume Phase 12 tokens (`MMHBCard` `resting`/`elevated`, `MMHBButton` `primary`/`tertiary`, `colors.semantic.{fgBody,fgMuted,bgCard,borderSubtle,statusWarning}`, `typography.fonts.{heading,body}`, `radius.{md,lg}`, `spacing.{xs,sm}`) — **zero hex literals** in components.

**Testing — 28/28 PASS in 543ms** via isolated `tests/vitest.config.mjs` (same isolation pattern as v5.8.51/v5.8.52/v5.8.53 — bypasses project-level setup that conflicts with running dev server on port 5000). 6 suites match spec exactly: Forbidden Patterns 9, Escalation 6, Tone Detection 5, Boundary Rules 2, Response Rotation 1, Conversation Router 5. Tests caught two real bugs during dev: "quiet" was in `calm` keywords and beat `grateful` on tie ordering for "I'm so thankful for the quiet" → removed `quiet`; elevated signal `i'm a burden` didn't match input `i'm such a burden` → added `such a burden` + `burden to everyone`. Module-boundary scan (manual ripgrep) confirms ZERO files outside `client/src/lumi-conversation/` import internals.

**Architect-driven hardening (post-build).** Architect review flagged that raw store mutators (`appendTurn`/`setEscalation`/`setStep`/`markPauseSuggested`) being barrel-exported allowed an adversarial host to inject Lumi turns directly and bypass the audit pipeline. Fixed in 3 changes: (1) added `submitUserTurn(userText)` in `runtime/conversationRouter.ts` — single hardened public write path that runs the full router, persists escalation through the safe API, and lands an audit-validated Lumi reply (or the verbatim crisis card on critical signal); (2) tightened the barrel — raw mutators are now INTENTIONALLY not re-exported from `index.ts`, only `useConversationStore` (read access), `submitUserTurn`, selectors, and types — store action types are documented as `INTERNAL` with explicit hosts-must-not-call notes; (3) added 3 adversarial contract tests verifying the public API path: benign input lands an audited Lumi turn, crisis input is short-circuited and `escalation: "critical"` is sticky across subsequent benign inputs, and 8 representative tone inputs all produce forbidden-safe Lumi output. Now **31/31 vitest tests PASS** (28 spec + 3 architect-hardening). Architect also flagged a documentation mismatch — spec lists tone-then-escalation but implementation runs escalation-before-tone for safety priority; resolved by codifying "escalation preempts tone" as authoritative in router doc comments rather than weakening crisis priority.

**Verification.** `tsc --noEmit` clean. `vite build` 15.51s clean. **31/31 vitest tests PASS** in 678ms via isolated `tests/vitest.config.mjs`. ZERO files outside `client/src/lumi-conversation/` modified. ZERO new deps. ZERO production wiring — opt-in module ready for future surface migration. Production import: `import { LumiConversationPanel, submitUserTurn } from "@/lumi-conversation"`. Spec checklist at `client/src/lumi-conversation/verification/phase15-checklist.md`.
