...
| 3 | **Landing pages** | `client/src/pages/CanvaLanding.jsx` (LIVE at `/` and `/canva-landing` per `App.jsx` L395-397 + L407, 1,397 lines, last polished v5.8.66) | QUARANTINED 2026-05-15: `client/src/pages/_quarantine/legacy-landing/Home.jsx`, `client/src/pages/_quarantine/legacy-landing/HealingLandingPage.jsx`. STILL LIVE (NOT quarantined): `client/src/pages/LandingV2.jsx` (mounted at `/landing-v2` per `App.jsx` L518) | **PARTIALLY_RESOLVED** — Iter 2e (2026-05-15): per user direction, attempted to quarantine all 3 legacy landings. **Refused on `CanvaLanding.jsx`** (LIVE homepage at `/` — quarantine would 404 the site) and **refused on `LandingV2.jsx`** (LIVE at `/landing-v2`). **Quarantined the 2 true orphans** (`Home.jsx` had no route + only `<Home>` references in code were the lucide-react icon, not the page; `HealingLandingPage.jsx` had no external import and no route). Both moved to `_quarantine/legacy-landing/` with relative imports rewritten `../` → `../../../`. tsc + vite build clean. The Iter 2d `<OfficialLumi>` swap on `Home.jsx` L195 was preserved through the move (governance-complete on a quarantined surface — for if/when it's restored). **Forward path:** `LandingV2.jsx` should be assessed (does `/landing-v2` need to keep working, or can the route + file be removed together?) before any further landing-page consolidation. |
