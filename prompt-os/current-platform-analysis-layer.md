...
**Location:** `prompt-os/current-platform-analysis-layer.md`
...
- server folder (entrypoint, routes, middleware, lib)
- client folder (src, components, pages, hooks, data)
...
- `.replit` matches `package.json`
- only one canonical runtime entrypoint
- routes do not hang

### 3. ARCHITECTURE ANALYSIS
Classify architecture by layer:
...
For each layer identify: current files, current functionality, missing components, duplicate systems, broken connections, smallest safe fix.

### 4. PRODUCT + UX ANALYSIS
Inspect homepage and user flows:
- clear brand hierarchy (GLP vs MMHB separation)
...
Inspect:
- blog structure, pillar pages, topic clusters
- metadata, headings, internal links, sitemap
- schema markup, search intent coverage
- newsletter capture, social repurposing

Classify each content item: `DISCOVERY | EDUCATION | REFLECTION | ACTION | CONTINUITY | COMMUNITY | CONVERSION`

Identify: missing pillars, weak search intent, disconnected content, unsafe claims.

### 6. AI + PROMPT SYSTEM ANALYSIS
Inspect:
- prompt files and prompt registry
- AI employees/agents and routing logic
- journaling prompts, reflection prompts
- safety boundaries, prompt duplication
- hallucination risk, clinical-risk language
- business/healing contamination

**Required rule:** AI must act as guided reflection infrastructure, not clinical authority.

Return: active prompts, duplicate prompts, unsafe patterns, canonical prompt structure.

### 7. BUSINESS / HEALING FIREWALL ANALYSIS
Check whether business logic appears inside:
- journaling, mood tracking, reflection flows
- crisis-safe areas, AI support prompts, onboarding
...
Return: contamination found/not found, files involved, risk level, extraction plan.

### 8. DESIGN SYSTEM ANALYSIS
Inspect: colors, typography, spacing, buttons, cards, shadows, radius, motion, icons, layout, accessibility contrast, mobile responsiveness.

Return: current tokens, missing tokens, hard-coded risks, inconsistencies, recommended token system.

**Preferred direction:** calm, clear, warm, accessible, trust-preserving, low cognitive load, nervous-system-aware.

### 9. DEPLOYMENT + OBSERVABILITY ANALYSIS
Inspect: deployment scripts, build scripts, logs, health checks, readiness checks, error handling, monitoring, test commands, rollback path.
...
Return: first failing gate, root cause, exact files, smallest safe patch, verification command, rollback command.

### 10. AVATAR + IDENTITY ANALYSIS
Inspect: canonical Lumi spec compliance, drift detection, image inventory, motion system, brand consistency.

Return: drift images (reject), canonical images (lock), assignment per context, protection rules.

---

## FINAL REPORT FORMAT (23 sections)

After analysis, produce a report in this exact order:
...
Run these to gather analysis inputs. The standard runner is `scripts/run-platform-analysis.sh` (idempotent — overwrites the snapshot in `docs/diagnostics/` each run).

```bash
mkdir -p docs/diagnostics

# Full directory tree (excluding node_modules, dist, build)
find . -maxdepth 3 \
...
# Runtime scan (ports, listeners, hosts)
...
# Domain scan (health, journal, mood, subscription, stripe)
...
echo "Platform analysis files created in docs/diagnostics/"
```
...
## RELATIONSHIP TO MMHB v7.4 ARCHIVAL KERNEL

This layer is **complementary** to `docs/governance/MMHB_v7.4_ARCHIVAL_KERNEL.md`:

- **Kernel** = WHAT must always be true (Primary Law, domain routing, BHCE, output contract).
- **This layer** = HOW to verify current state before any change satisfies the Kernel.

When the two conflict, the Kernel wins (it is the locked law). This layer's role is to surface the evidence the Kernel needs to make a safe decision.
