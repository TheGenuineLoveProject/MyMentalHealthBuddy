...
**Why:** A 2026-06 full frontend↔mount cross-ref mounted every `/api/*` prefix that had a matching `server/routes/*.mjs`. These remaining prefixes are called by the frontend but have NO route file at all, so they stay live 404s until a backend is written: `/api/user-settings`, `/api/dashboard`, `/api/uploads`, `/api/social`, `/api/v1/*` (rbac-gated, aspirational).
**How to apply:** these are real gaps but bigger than a mount — each needs a route module + storage. Confirm product intent before building; some (`/api/v1`, `/api/social`) may be aspirational.
