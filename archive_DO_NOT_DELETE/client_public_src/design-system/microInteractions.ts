...
export function getTransition(
  ...
  speed: keyof typeof transitions = 'base'
): string {
...
export const focusStates = {
  ...
  therapeutic: (mode: 'serenity' | 'empowerment' | 'focus' | 'recovery') => {
    const modeColors = {
    ...
    const colorWithAlpha = (hslColor: string) => {
 ...
 */
export function getStaggerConfig(itemCount: number, delay: number = 50) {
  if (prefersReducedMotion()) {
...
export const cssHelpers = {
   ...
   */
  hoverLift: (enabled: boolean = true) => {
   ...
   */
  focusRing: (mode?: 'serenity' | 'empowerment' | 'focus' | 'recovery') => {
   ...
   */
  smoothTransition: (properties: string[] = ['all'], speed: keyof typeof transitions = 'base') => {
    const config = transitions[speed];
