 ...
 */
export function addPreconnect(origins: string[], crossOrigin = false): void {
 ...
 */
export function preloadResource(hints: ResourceHint[]): void {
  hints.forEach((hint) => {
    const link = document.createElement('link');
    link.rel = 'preload';
    link.href = hint.href;
    ...
    if (hint.priority) link.setAttribute('importance', hint.priority);
    document.head.appendChild(link);
  });
}
 ...
 */
export function prefetchResource(urls: string[]): void {
 ...
 */
export function lazyLoadImages(selector = '[data-lazy]'): void {
 ...
 */
export function optimizeFontLoading(fonts: { family: string; weight?: string; style?: string }[]): void {
  if ('fonts' in document) {
    fonts.forEach((font) => {
      const fontFace = `${font.weight || '400'} ${font.style || 'normal'} ${font.family}`;
      (document as any).fonts.load(fontFace).catch((err: Error) => {
