  ...
  return (
    <article className="card-glass p-6 hover:shadow-lg transition-shadow" data-testid={`card-post-${post.id}`}>
      ...
      <Link href={`/blog/${post.slug}`}>
        <h2 className="text-xl font-semibold text-[var(--glp-sage-deep)] hover:text-[#8FBF9F] transition-colors cursor-pointer" data-testid={`link-post-title-${post.id}`}>
          {post.title}
        </h2>
      </Link>
      ...
      <Link href={`/blog/${post.slug}`}>
        <button className="mt-4 flex items-center gap-2 text-sm font-medium text-[var(--glp-sage-deep)] hover:text-[#8FBF9F] transition-colors" data-testid={`button-read-more-${post.id}`}>
          ...
          <div className="relative">
            ...
            <div className="mt-6 relative max-w-md">
              ...
              <input
                ...
                className="w-full pl-12 pr-4 py-3 rounded-2xl border border-[rgba(var(--glp-sage-deep-rgb), 0.25)] bg-white/70 text-sm focus:outline-none focus:ring-2 focus:ring-[#8FBF9F]/50"
