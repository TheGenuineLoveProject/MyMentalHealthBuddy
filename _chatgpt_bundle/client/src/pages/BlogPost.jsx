    ...
    <div className="border-l-2 border-[rgba(143,191,159,0.3)] pl-4 py-3" data-testid={`comment-${comment.id}`}>
      ...
      {user && (
        <button
          ...
          className="mt-2 flex items-center gap-1 text-xs text-[var(--glp-sage-deep)] hover:text-[#8FBF9F] transition-colors"
      ...
      {showReplyForm && (
        <div className="mt-3 flex gap-2">
          <input
            ...
            className="flex-1 px-3 py-2 rounded-lg border border-[rgba(var(--glp-sage-deep-rgb), 0.25)] bg-white/70 text-sm focus:outline-none focus:ring-2 focus:ring-[#8FBF9F]/50"
            data-testid={`input-reply-${comment.id}`}
          />
          <button
            onClick={handleReply}
            disabled={replyMutation.isPending}
            className="px-3 py-2 rounded-lg bg-[var(--glp-sage-deep)] text-white text-sm hover:bg-[#8FBF9F] transition-colors disabled:opacity-50"
      ...
      {user ? (
        <div className="mb-8">
          <textarea
            ...
            rows={3}
            className="w-full px-4 py-3 rounded-xl border border-[rgba(var(--glp-sage-deep-rgb), 0.25)] bg-white/70 text-sm focus:outline-none focus:ring-2 focus:ring-[#8FBF9F]/50 resize-none"
            data-testid="textarea-new-comment"
          />
          <button
            onClick={handleSubmit}
            disabled={commentMutation.isPending || !newComment.trim()}
            className="mt-3 px-6 py-2 rounded-xl bg-[var(--glp-sage-deep)] text-white text-sm font-medium hover:bg-[#8FBF9F] transition-colors disabled:opacity-50 flex items-center gap-2"
    ...
    return (
      <div className="min-h-screen bg-[var(--glp-paper)]">
        ...
        <div className="max-w-3xl mx-auto px-4 py-20 text-center">
          ...
          <Link href="/blog">
            <button className="mt-6 px-6 py-3 rounded-xl bg-[var(--glp-sage-deep)] text-white text-sm font-medium hover:bg-[#8FBF9F] transition-colors" data-testid="button-back-to-blog">
              Back to Blog
            </button>
          </Link>
        </div>
  ...
  return (
    <div className="min-h-screen bg-[var(--glp-paper)]">
      ...
      <main className="max-w-3xl mx-auto px-4 py-10">
        <Link href="/blog">
          <button className="flex items-center gap-2 text-sm text-[var(--glp-sage-deep)] hover:text-[#8FBF9F] transition-colors mb-8" data-testid="button-back">
