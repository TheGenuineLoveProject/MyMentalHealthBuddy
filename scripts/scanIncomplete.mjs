...
function scanFile(filePath) {
  try {
    ...
    if (isWellness) {
      ...
      if (!WELLNESS_CHECKS.clarityCard.test(content)) {
        issues.missingClarityCard.push({ file: relativePath });
      }
      if (!WELLNESS_CHECKS.examplesAccordion.test(content)) {
        issues.missingExamplesAccordion.push({ file: relativePath });
      }
    }
  } catch (err) {
...
function generateReport() {
  ...
  const totalIssues = 
    ...
    issues.missingClarityCard.length +
    issues.missingExamplesAccordion.length;
  ...
  report += `| Missing ClarityCard (wellness) | ${issues.missingClarityCard.length} |\n`;
  report += `| Missing ExamplesAccordion (wellness) | ${issues.missingExamplesAccordion.length} |\n`;
  ...
  if (issues.missingClarityCard.length > 0) {
    report += `## Wellness Pages Missing ClarityCard\n\n`;
    report += `ClarityCard provides What/Who/When/Why/How/Where explanation for each tool.\n\n`;
    issues.missingClarityCard.forEach(({ file }) => {
      report += `- ${file}\n`;
    });
    report += `\n`;
  }

  if (issues.missingExamplesAccordion.length > 0) {
    report += `## Wellness Pages Missing ExamplesAccordion\n\n`;
    report += `ExamplesAccordion shows Beginner/Intermediate/Advanced examples.\n\n`;
    issues.missingExamplesAccordion.forEach(({ file }) => {
      report += `- ${file}\n`;
    });
    report += `\n`;
  }
...
function main() {
  ...
  console.log(`   Missing ClarityCard: ${issues.missingClarityCard.length}`);
  console.log(`   Missing ExamplesAccordion: ${issues.missingExamplesAccordion.length}`);
