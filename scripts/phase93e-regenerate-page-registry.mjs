...
fs.mkdirSync("diagnostics/phase93e", { recursive: true });
...
const registry = rows.map(([path, title, domain, priority, intent]) => ({
  path,
  title,
  domain,
  priority,
  intent,
  sitemap: true,
  presentInApp: path === "/" ? true : app.includes(`path="${path}"`) || app.includes(`path='${path}'`),
}));

const missing = registry.filter((route) => !route.presentInApp);
const sitemapBody = registry
  .filter((route) => route.presentInApp)
  .map((route) => `  <url>
...
fs.writeFileSync("diagnostics/phase93e/canonical-page-registry.json", JSON.stringify(registry, null, 2));
fs.writeFileSync("diagnostics/phase93e/registry-summary.json", JSON.stringify({ siteUrl, missing, total: registry.length }, null, 2));
