...
export async function repairDuplicates({ root, extensionsToRemove, exclude }: { root: string; extensionsToRemove: string[]; exclude: string[] }) {
  const files = await fs.readdir(root, { recursive: true });
  const deletions = files.filter((f: any) =>
...
export async function registerMissingRoutes({ routesFile, modules }: { routesFile: string; modules: { path: string, endpoint: string }[] }) {
...
export async function deployOpenAI({ fallbackOnly, envPath, model }: { fallbackOnly: boolean, envPath: string, model: string }) {
...
export async function enforceSchema({ schemaPath, drizzleConfig }: { schemaPath: string, drizzleConfig: string }) {
  const exec = require('child_process').exec;
  exec(`npx drizzle-kit push --schema=${schemaPath} --config=${drizzleConfig}`, (err: any) => {
    if (err) console.error('❌ Drizzle push failed:', err);
  });
}

export async function assignAIEmployees({ aiList, components }: { aiList: string[], components: string[] }) {
...
export async function activateHealingUI({ layoutPath, healingComponent }: { layoutPath: string, healingComponent: string }) {
