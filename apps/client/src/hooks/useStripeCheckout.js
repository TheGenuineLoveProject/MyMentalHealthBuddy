...
export function useStripeCheckout() {
    ...
    const createSubscriptionCheckout = useMutation({
        mutationFn: async ({ tier, successUrl, cancelUrl, }) => {
    ...
    const createOneTimeCheckout = useMutation({
        mutationFn: async ({ successUrl, cancelUrl, }) => {
            const baseUrl = window.location.origin;
    ...
    return {
        ...
        redirectToCheckout,
        isLoading: createSubscriptionCheckout.isPending || createOneTimeCheckout.isPending,
        error,
        clearError,
    };
}
