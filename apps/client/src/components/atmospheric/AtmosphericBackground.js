...
export function AtmosphericBackground({ scene = 'default', intensity = 'moderate', showParticles = true, className = '', }) {
    ...
    return (_jsxs("div", { className: `fixed inset-0 -z-10 overflow-hidden ${className}`, "aria-hidden": "true", children: [_jsx("div", { className: "absolute inset-0 transition-opacity duration-1000", style: {
                    background: `linear-gradient(135deg, ${currentGradient.from} 0%, ${currentGradient.via} 50%, ${currentGradient.to} 100%)`,
                } }), !reducedMotion && (_jsx("div", { className: "absolute inset-0 opacity-70", style: {
                    ...animationStyle,
                    background: `radial-gradient(ellipse at top, ${currentGradient.accent} 0%, transparent 60%)`,
                } })), showParticles && !reducedMotion && (_jsx(ParticleField, { scene: scene, intensity: intensity })), _jsx("div", { className: "absolute inset-0 opacity-[0.015] mix-blend-overlay pointer-events-none", style: {
 ...
 */
function ParticleField({ scene, intensity, }) {
    ...
    return (_jsx("div", { className: "absolute inset-0 overflow-hidden pointer-events-none", children: particles.map((particle) => (_jsx("div", { className: "absolute rounded-full bg-gradient-to-br from-white/40 to-white/10 blur-sm", style: {
 ...
 */
export function DecorativeWave({ position = 'top', scene = 'serenity', className = '', }) {
    const reducedMotion = useReducedMotion();
    const colors = {
    ...
    return (_jsx("div", { className: `absolute ${position === 'top' ? 'top-0' : 'bottom-0'} left-0 right-0 h-32 overflow-hidden pointer-events-none ${className}`, "aria-hidden": "true", children: _jsx("svg", { className: "absolute w-full h-full", viewBox: "0 0 1440 320", preserveAspectRatio: "none", style: {
                ...animationStyle,
                transform: position === 'bottom' ? 'scaleY(-1)' : 'none',
            }, children: _jsx("path", { fill: colors[scene], d: "M0,96L48,112C96,128,192,160,288,160C384,160,480,128,576,122.7C672,117,768,139,864,154.7C960,171,1056,181,1152,165.3C1248,149,1344,107,1392,85.3L1440,64L1440,320L1392,320C1344,320,1248,320,1152,320C1056,320,960,320,864,320C768,320,672,320,576,320C480,320,384,320,288,320C192,320,96,320,48,320L0,320Z" }) }) }));
}
