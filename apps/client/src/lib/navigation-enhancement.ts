...
export class CommandPalette {
  private commands: Map<string, NavigationItem> = new Map();
  ...
  private registerDefaultCommands(): void {
    const defaultCommands: NavigationItem[] = [
      {
        id: 'nav-dashboard',
   ...
   */
  private calculateScore(command: NavigationItem, query: string): number {
    let score = 0;
   ...
   */
  getRecentCommands(): SearchResult[] {
    return this.recentlyUsed
      .map(id => this.commands.get(id))
      .filter((cmd): cmd is NavigationItem => cmd !== undefined)
      .slice(0, 5)
      .map(item => ({
        item,
        score: 100,
   ...
   * Track command usage
   */
  private trackUsage(commandId: string): void {
   ...
   */
  registerCommand(command: NavigationItem): void {
...
export class KeyboardShortcutsManager {
   ...
   */
  private matchesSingleKey(combo: string, key: string): boolean {
