 ...
 */
async function checkDatabase(storageInstance: any): Promise<HealthStatus> {
 ...
 */
async function checkOpenAI(): Promise<HealthStatus | undefined> {
 ...
 */
export async function performHealthCheck(storageInstance?: any): Promise<HealthCheckResult> {
  const checks = await Promise.all([
    storageInstance ? checkDatabase(storageInstance) : Promise.resolve({ status: 'warn' as const, message: 'Database check skipped' }),
 ...
 */
export function healthCheckHandler(storageInstance?: any) {
 ...
 */
export function readinessHandler(storageInstance?: any) {
  return async (req: Request, res: Response) => {
    try {
      const dbCheck = storageInstance ? await checkDatabase(storageInstance) : { status: 'warn' as const };
