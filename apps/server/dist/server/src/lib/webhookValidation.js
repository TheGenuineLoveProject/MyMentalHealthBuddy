 ...
 */
export function validateStripeWebhook(stripe) {
    return (req, res, next) => {
...
export function ensureWebhookIdempotency(req, res, next) {
 ...
 */
export function validateWebhookEventType(allowedTypes) {
    return (req, res, next) => {
        const event = req.body;
        if (!event || !event.type) {
            return res.status(400).json({
