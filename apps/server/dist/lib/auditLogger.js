...
exports.logContentChange = logContentChange;
 ...
 */
async function logAudit(entry, storageInstance) {
    try {
 ...
 */
async function logAuth(action, req, userId, details) {
 ...
 */
async function logDataAccess(action, resourceType, resourceId, req, userId) {
    const clientInfo = getClientInfo(req);
    await logAudit({
        userId,
 ...
 */
async function logPayment(action, req, userId, details) {
    const clientInfo = getClientInfo(req);
    await logAudit({
        userId,
        action,
 ...
 * Log admin action
 */
async function logAdminAction(action, req, userId, targetResourceType, targetResourceId, changes) {
    const clientInfo = getClientInfo(req);
    await logAudit({
        userId,
 ...
 */
async function logContentChange(action, resourceType, resourceId, req, userId, changes) {
    const clientInfo = getClientInfo(req);
    await logAudit({
        userId,
        action: `${resourceType}_${action}`,
 ...
 */
async function logAIUsage(action, req, userId, details) {
    const clientInfo = getClientInfo(req);
    await logAudit({
        userId,
        action: `ai_${action}`,
