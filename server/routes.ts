...
import { cacheMiddleware, apiCache, healthCache, getCacheStats, clearAllCaches } from "./services/cache";
...
export function setupRoutes(app: Express, db: any): void {
  ...
  app.get("/api/services", cacheMiddleware(apiCache, 60), asyncHandler(async (req, res) => {
    const services = await storage.getAllServices();
    res.json(services);
  }));
  ...
  app.get("/api/endpoints", cacheMiddleware(apiCache, 300), asyncHandler(async (req, res) => {
    const endpoints = await storage.getAllApiEndpoints();
    res.json(endpoints);
  }));

  // Get project structure with caching
  app.get("/api/structure", cacheMiddleware(apiCache, 600), asyncHandler(async (req, res) => {
    const structure = await storage.getAllProjectStructure();
    res.json(structure);
  }));

  // Get all packages with caching
  app.get("/api/packages", cacheMiddleware(apiCache, 600), asyncHandler(async (req, res) => {
    const packages = await storage.getAllPackages();
    res.json(packages);
  }));

  // Get all scripts with caching
  app.get("/api/scripts", cacheMiddleware(apiCache, 600), asyncHandler(async (req, res) => {
    const scripts = await storage.getAllScripts();
    res.json(scripts);
  }));

  // Test endpoint connectivity
  app.post("/api/test-endpoint", asyncHandler(async (req, res) => {
    const { method, path } = req.body;
    
    if (!method || !path) {
      throw new ValidationError("Method and path are required for endpoint testing");
    }
  ...
  app.post("/api/healing-employee", asyncHandler(async (req, res) => {
    ...
    try {
      ...
      try {
        ...
        console.error("Storage error while saving healing message:", storageError);
  ...
  app.post("/api/remove-duplicates", asyncHandler(async (req, res) => {
  ...
  if (config.ENABLE_API_DOCS) {
    ...
    app.get("/api/docs/openapi", asyncHandler(async (req, res) => {
      ...
      const openApiSpec = {
        ...
        paths: apiDocumentation.endpoints.reduce((paths, endpoint) => {
          ...
          paths[pathKey][endpoint.method.toLowerCase()] = {
            ...
            responses: Object.entries(endpoint.responses).reduce((responses, [code, response]) => {
              responses[code] = {
