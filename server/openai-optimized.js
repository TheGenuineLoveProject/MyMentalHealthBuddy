...
try {
  ...
  console.warn(
    "⚠️ OpenAI module not available, using enhanced fallback system"
  );
}
...
export async function generateHealingResponse(
  ...
  const contextHash = JSON.stringify(conversationHistory || []).substring(
    0,
    100
  );
  const cacheKey = getCacheKey(
    "ai-response-v2",
    `${userMessage.toLowerCase().trim()}-${contextHash}`
  );
  ...
  try {
    ...
    console.log(
      `✅ AI response generated (${response.usage?.total_tokens || 0} tokens)`
    );
    return aiResponse;
  } catch (error) {
...
export function generateEnhancedFallback(userMessage, reason) {
  ...
  if (reason === "rate-limit") {
    prefix =
      "I'm experiencing high demand right now, but I'm still here for you. ";
  } else if (reason === "service-error") {
    prefix =
      "While my advanced features are temporarily limited, I want you to know that ";
  }
