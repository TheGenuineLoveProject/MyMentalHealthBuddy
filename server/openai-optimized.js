...
function detectCommonTopic(message) {
...
export async function generateHealingResponse(userMessage, conversationHistory, sessionId, options) {
    // Rate limiting check
    ...
    try {
        ...
        const response = await retryWithBreaker(async () => openai.chat.completions.create(completionOptions), openAIBreaker, retryConfigs.critical);
        // Track token usage
        if (response.usage) {
...
export function generateEnhancedFallback(userMessage, reason) {
...
export function generateCompassionateFallback(userMessage) {
    const keywords = userMessage.toLowerCase();
    // 🆘 Crisis
    if (keywords.includes('hurt myself') ||
...
export async function* streamHealingResponse(userMessage, sessionId) {
    if (!openai || !process.env.OPENAI_API_KEY) {
