...
export class AdvancedOpenAI extends EventEmitter {
  ...
  private async generateIntelligentResponse(params: any) {
    ...
    const systemContext = params.messages.find((m: any) => m.role === 'system')?.content || '';
    const userMessage = params.messages[params.messages.length - 1]?.content || '';
    
    let content = this.generateContextualResponse(systemContext, userMessage);
  ...
  private generateContextualResponse(systemContext: string, userMessage: string): string {
  ...
  private enableIntelligentCaching() {
    ...
    setInterval(() => {
      ...
      console.log(`🧹 [Advanced OpenAI] Cache cleaned. Size: ${this.responseCache.size}`);
