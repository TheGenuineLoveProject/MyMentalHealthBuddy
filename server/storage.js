...
export class MemStorage {
    ...
    initializeData() {
        ...
        const endpoints = [
            { method: "GET", path: "/api/health", description: "Health check and system status", status: "active" },
            { method: "GET", path: "/api/services", description: "Get all running services", status: "active" },
            { method: "GET", path: "/api/endpoints", description: "Get all API endpoints", status: "active" },
            { method: "GET", path: "/api/structure", description: "Get project structure", status: "active" },
            { method: "GET", path: "/api/packages", description: "Get installed packages", status: "active" },
            { method: "GET", path: "/api/scripts", description: "Get available npm scripts", status: "active" },
            { method: "POST", path: "/api/healing-ai", description: "AI therapeutic response endpoint", status: "active" },
            { method: "POST", path: "/api/healing-employee", description: "AI healing employee endpoint", status: "active" },
            { method: "POST", path: "/api/remove-duplicates", description: "Remove duplicate data entries", status: "active" },
        ];
        endpoints.forEach(endpoint => {
            const apiEndpoint = { ...endpoint, id: randomUUID(), status: endpoint.status || "active" };
        ...
        const structure = [
            ...
            { name: "package.json", type: "file", path: "/package.json", parentId: null },
            { name: "vite.config.ts", type: "file", path: "/vite.config.ts", parentId: null },
            { name: "tailwind.config.ts", type: "file", path: "/tailwind.config.ts", parentId: null },
        ];
        structure.forEach(item => {
            const structureItem = { ...item, id: randomUUID(), parentId: item.parentId || null };
            this.projectStructure.set(structureItem.id, structureItem);
        });
        // Initialize packages - Core dependencies only
        const packageList = [
            { name: "express", version: "^4.21.2", environment: "backend", isDev: false },
            { name: "react", version: "^18.3.1", environment: "frontend", isDev: false },
            { name: "vite", version: "^7.1.6", environment: "fullstack", isDev: true },
            { name: "@tanstack/react-query", version: "^5.60.5", environment: "frontend", isDev: false },
            { name: "tailwindcss", version: "^3.4.17", environment: "frontend", isDev: true },
            { name: "typescript", version: "5.6.3", environment: "fullstack", isDev: true },
            { name: "openai", version: "^5.22.0", environment: "backend", isDev: false },
            { name: "drizzle-orm", version: "^0.39.1", environment: "backend", isDev: false },
        ];
        packageList.forEach(pkg => {
            const packageItem = { ...pkg, id: randomUUID(), isDev: pkg.isDev || false };
            this.packages.set(packageItem.id, packageItem);
        });
        // Initialize scripts - Actual npm scripts
        const scriptList = [
            { name: "dev", command: "npm run dev", description: "Start development server (Express + Vite)", environment: "fullstack" },
            { name: "build", command: "npm run build", description: "Build for production", environment: "fullstack" },
            { name: "start", command: "npm run start", description: "Start production server", environment: "fullstack" },
            { name: "check", command: "npm run check", description: "TypeScript type checking", environment: "fullstack" },
            { name: "db:push", command: "npm run db:push", description: "Push database schema changes", environment: "backend" },
        ];
        scriptList.forEach(script => {
            const scriptItem = { ...script, id: randomUUID(), description: script.description || null };
    ...
    async createService(service) {
        ...
        const existing = Array.from(this.services.values()).find(s => s.name === service.name && s.port === service.port);
        if (existing) {
    ...
    async createApiEndpoint(endpoint) {
        ...
        const existing = Array.from(this.apiEndpoints.values()).find(e => e.method === endpoint.method && e.path === endpoint.path);
        if (existing) {
            // Update existing instead of creating duplicate
            const updated = { ...existing, ...endpoint, status: endpoint.status || existing.status };
            this.apiEndpoints.set(existing.id, updated);
            return updated;
        }
        const id = randomUUID();
        const newEndpoint = { ...endpoint, id, status: endpoint.status || "active" };
    ...
    async createProjectStructure(item) {
        ...
        const existing = Array.from(this.projectStructure.values()).find(i => i.path === item.path);
        if (existing) {
            // Update existing instead of creating duplicate
    ...
    async createPackage(pkg) {
        ...
        const existing = Array.from(this.packages.values()).find(p => p.name === pkg.name && p.environment === pkg.environment);
        if (existing) {
    ...
    async createScript(script) {
        ...
        const existing = Array.from(this.scripts.values()).find(s => s.name === script.name && s.command === script.command);
        if (existing) {
            // Update existing instead of creating duplicate
        ...
        const id = randomUUID();
        const newScript = { ...script, id, description: script.description || null };
    ...
    async removeDuplicates() {
        ...
        this.services.forEach((service, id) => {
            ...
            if (servicesSeen.has(key)) {
                ...
                details.push(`Removed duplicate service: ${service.name} on port ${service.port}`);
                removed++;
            }
            else {
        ...
        this.apiEndpoints.forEach((endpoint, id) => {
            ...
            if (endpointsSeen.has(key)) {
                ...
                details.push(`Removed duplicate endpoint: ${endpoint.method} ${endpoint.path}`);
                removed++;
            }
            else {
        ...
        this.packages.forEach((pkg, id) => {
            ...
            if (packagesSeen.has(key)) {
                ...
                details.push(`Removed duplicate package: ${pkg.name} in ${pkg.environment}`);
                removed++;
            }
            else {
