...
export async function generateHealingResponse(userMessage) {
    if (!openai || !process.env.OPENAI_API_KEY) {
    ...
    try {
        const response = await retryWithBreaker(async () => openai.chat.completions.create({
          ...
          Always maintain a professional therapeutic boundary while being genuinely caring.`,
            ...
            temperature: 0.7,
            max_tokens: 200,
        }), openAIBreaker, retryConfigs.critical);
