...
export interface IStorage {
  ...
  createProjectStructure(item: InsertProjectStructure): Promise<ProjectStructure>;
  ...
  getHealingMessagesBySessionId(sessionId: string, userId: string): Promise<HealingMessage[]>;
  updateHealingMessageFeedback(id: string, userId: string, isHelpful: boolean, feedback?: string): Promise<void>;
...
export class MemStorage implements IStorage {
  ...
  private initializeData() {
    ...
    endpoints.forEach(endpoint => {
      const apiEndpoint: ApiEndpoint = { ...endpoint, id: randomUUID(), status: endpoint.status || "active" };
    ...
    structure.forEach(item => {
      const structureItem: ProjectStructure = { ...item, id: randomUUID(), parentId: item.parentId || null };
    ...
    packageList.forEach(pkg => {
      const packageItem: Package = { ...pkg, id: randomUUID(), isDev: pkg.isDev || false };
  ...
  async createProjectStructure(item: InsertProjectStructure): Promise<ProjectStructure> {
  ...
  async getHealingMessagesBySessionId(sessionId: string, userId: string): Promise<HealingMessage[]> {
    return Array.from(this.healingMessages.values())
      .filter(msg => msg.sessionId === sessionId && msg.userId === userId)
      .sort((a, b) => new Date(a.timestamp).getTime() - new Date(b.timestamp).getTime());
  }

  async updateHealingMessageFeedback(id: string, userId: string, isHelpful: boolean, feedback?: string): Promise<void> {
