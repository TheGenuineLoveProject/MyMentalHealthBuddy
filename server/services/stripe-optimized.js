...
export async function createOrUpdateCustomer(userId, email, metadata) {
    if (!stripe)
        throw new Error('Stripe not configured');
    const cacheKey = getCacheKey('stripe-customer', userId);
    const cachedCustomerId = aiResponseCache.get(cacheKey);
    if (cachedCustomerId) {
    ...
    try {
        ...
        // Create new customer
        const customer = await retryWithBreaker(async () => stripe.customers.create({
            email,
            metadata: {
                ...metadata,
                userId,
        ...
        }), stripeBreaker, retryConfigs.standard);
...
export async function createSubscription(userId, email, tier, options) {
    if (!stripe)
        throw new Error('Stripe not configured');
    ...
    try {
        ...
        await storage.updateUserSubscription(userId, tier, subscription.status, subscription.current_period_end
...
export async function handleWebhookEvent(event, idempotencyKey) {
...
function detectTierFromPrice(subscription) {
...
export async function getSubscriptionDetails(userId) {
    ...
    try {
        ...
        let subscriptionData = {
            ...
            features: SUBSCRIPTION_TIERS[user.subscriptionTier || 'free'].features,
            usage: {
...
export async function trackAIUsage(userId) {
    ...
    const limit = tierConfig.features.aiSessions;
