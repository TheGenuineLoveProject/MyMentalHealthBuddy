...
export class AIMonitoringSystem extends EventEmitter {
    ...
    collectAIMetrics() {
        ...
        this.recordMetric({
            ...
            value: this.counters.cacheHits / Math.max(1, this.counters.cacheHits + this.counters.cacheMisses),
            unit: 'ratio',
            timestamp: Date.now()
        });
    ...
    collectPerformanceMetrics() {
        ...
        const errorRate = this.counters.errors / Math.max(1, this.counters.requests);
        this.recordMetric({
    ...
    checkAlertConditions(metric) {
        ...
        // Check error rate
        if (metric.name === 'http.errors.rate' && metric.value > thresholds.errorRate) {
            this.createAlert({
                level: 'warning',
        ...
        // Check AI accuracy
        if (metric.name === 'ai.model.accuracy' && metric.value < thresholds.aiAccuracy) {
            this.createAlert({
                level: 'error',
    ...
    reviewAlerts() {
        ...
        this.alerts.forEach(alert => {
            if (!alert.resolved && alert.metric && alert.threshold !== undefined) {
                ...
                if (currentMetric) {
                    const isResolved = alert.metric.includes('rate') || alert.metric.includes('usage')
    ...
    generateReport() {
        const report = {
            ...
            metrics: Object.fromEntries(Array.from(this.metrics.entries()).map(([name, metrics]) => [
                name,
                {
