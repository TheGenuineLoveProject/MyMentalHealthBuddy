...
export class AILearningEngine extends EventEmitter {
  ...
  private calculateGradient(weightKey: string, data: LearningData, reward: number): number {
  ...
  provideFeedback(experienceId: string, feedback: string, rating: number) {
    const experience = this.learningHistory.find(e => 
      `${e.timestamp}_${e.context}_${e.action}` === experienceId
    );
