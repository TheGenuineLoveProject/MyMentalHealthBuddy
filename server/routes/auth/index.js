...
const generateTokens = (userId) => {
    const accessToken = jwt.sign({ userId }, process.env.JWT_SECRET || 'mymentalhealthbuddy-jwt-secret-2024', { expiresIn: '24h' });
    const refreshToken = jwt.sign({ userId, type: 'refresh' }, process.env.JWT_REFRESH_SECRET || 'mymentalhealthbuddy-refresh-secret-2024', { expiresIn: '30d' });
...
router.post('/refresh', asyncHandler(async (req, res) => {
    ...
    try {
        const decoded = jwt.verify(refreshToken, process.env.JWT_REFRESH_SECRET || 'mymentalhealthbuddy-refresh-secret-2024');
