...
// Generate TTS audio
router.post('/generate', optionalAuthenticateToken as any, asyncHandler(async (req, res) => {
...
router.post('/stream', optionalAuthenticateToken as any, asyncHandler(async (req, res) => {
  const validation = ttsRequestSchema.safeParse(req.body);
  if (!validation.success) {
    throw new ValidationError(validation.error.issues.map((e: any) => e.message).join(', '));
  }
