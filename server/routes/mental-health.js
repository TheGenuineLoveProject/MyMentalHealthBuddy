...
router.post("/chat", asyncHandler(async (req, res) => {
    try {
        ...
        if (sessionId) {
            try {
                previousMessages = await storage.getHealingMessagesBySessionId(sessionId, userId);
            }
            catch (error) {
        ...
        try {
            if (!process.env.OPENAI_API_KEY) {
                ...
                aiResponse = await generateHealingResponse(message, conversationHistory);
            }
        }
        catch (error) {
        ...
        const fallbackResponse = generateCompassionateFallback(req.body.message || "");
        res.json({
            success: true,
...
router.post("/feedback/:messageId", asyncHandler(async (req, res) => {
    const userId = req.session?.user?.id;
    ...
    try {
        await storage.updateHealingMessageFeedback(messageId, userId, isHelpful, feedback);
...
router.get("/crisis-resources", asyncHandler(async (req, res) => {
    res.json({
        success: true,
        resources: [
            {
