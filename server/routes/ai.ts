...
import { z } from 'zod';
import { authenticateToken, optionalAuthenticateToken, type AuthRequest } from '../auth/jwt';
...
router.get('/conversations', authenticateToken as any, asyncHandler(async (req: AuthRequest, res) => {
  if (!req.user) {
...
router.get('/conversations/:sessionId', authenticateToken as any, asyncHandler(async (req: AuthRequest, res) => {
  if (!req.user) {
    throw new ValidationError('User not found');
  }
...
router.post('/chat', optionalAuthenticateToken as any, asyncHandler(async (req: AuthRequest, res) => {
  const validation = healingChatSchema.safeParse(req.body);
  if (!validation.success) {
    throw new ValidationError(validation.error.issues.map((e: any) => e.message).join(', '));
  }
...
router.post('/conversations/:id/feedback', authenticateToken as any, asyncHandler(async (req: AuthRequest, res) => {
  if (!req.user) {
    throw new ValidationError('User not found');
  }
