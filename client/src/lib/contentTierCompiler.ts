...
export type ContentLevel = 'beginner' | 'standard' | 'deep';
...
export function selectByLevel<T>(content: Record<ContentLevel, T>, level: ContentLevel): T {
  return content[level] || content.standard;
}

export function createTieredDescription(concept: string): TieredContent {
  return {
    beginner: `A simple way to help you feel ${concept}.`,
    standard: `A practice that may support ${concept} through gentle awareness.`,
    deep: `An evidence-informed approach to ${concept}, integrating somatic and cognitive elements.`
  };
}
...
export function createTieredSteps(steps: string[]): TieredContent[] {
  return steps.map((step, idx) => ({
    ...
    standard: step,
    deep: `Step ${idx + 1}: ${step} This supports nervous system regulation.`
  }));
}
