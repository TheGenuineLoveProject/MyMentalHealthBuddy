...
- **Placement (v2.0 / v2.3)**: `<BuddyPanel>` renders at 140px on `/start` (centerpiece) and 88px in calm baseline on `/journal`, `/state`, and `/pathways/onboarding` (work surfaces — calm/decision-making contexts). Tool pages (`/breathing`, `/grounding`, etc.) are intentionally NOT instrumented (over-placement guardrail).
...
- **Centralized companion copy (v2.2 / v2.3)**: `BUDDY_PANEL_COPY` export in `client/src/content/microcopy/wellnessMicrocopy.ts` is the single source of truth for `<BuddyPanel>` title/subtitle strings on calm work surfaces. Keys mirror the telemetry `surface` prop (`journal`, `mood`, `onboarding`). `/start`'s contextual title is intentionally excluded (computed dynamically from return-user / first-touch state).
- **Heading semantics (v2.4)**: `<BuddyPanel>` accepts a `titleAs?: "h2" | "p"` prop. Default `"h2"` preserves the `/start` hero pattern (Buddy IS the page heading). Companion adopters where the host page owns its own `<h1>` (`/journal`, `/state`, `/pathways/onboarding`) pass `titleAs="p"` so the panel renders a styled paragraph instead of an `<h2>`, leaving the page's `<h1>` as the unambiguous primary heading in the document outline. Both branches use identical Tailwind classes and `data-testid` so visual presentation and e2e selectors are byte-identical.
- **Heading-contract regression guard (v2.7)**: `scripts/check-buddy-heading-contract.mjs` is a standalone Node invariant script (joins the existing `scripts/check-contract-routes.sh` pre-test gate pattern) that asserts at the source level: (1) BuddyPanel exposes the `titleAs` prop with default `"h2"`, both branches present, identical classes/testids; (2) every companion adopter passes `titleAs="p"` and still owns its own `<h1>`; (3) `/start` does NOT pass `titleAs="p"`. Mirrored as `tests/unit/buddyPanelHeadingContract.test.mjs` for CI integration. Either layer fails loudly the moment a contributor removes the prop, removes a branch, or forgets `titleAs="p"` on a new companion surface. Joined into `scripts/check-contract-routes.sh` so the route-registry check + heading-contract check run as a single architectural gate.
- **Placement-geometry + surface↔library-alignment guard (v2.8)**: `scripts/check-buddy-placement-contract.mjs` is a sibling Node invariant script (single-responsibility — separate file from the v2.7 heading guard so each architectural rule has its own focused failure surface). Asserts at the source level: (1) every companion adopter passes `state="calm"` (no inferred emotion on work surfaces — locks the v2.0 placement contract); (2) every companion adopter passes `size={88}` (calm-companion size, not hero); (3) every companion adopter's `surface="<value>"` is a valid key in `BUDDY_PANEL_COPY` (catches "added a surface, forgot the copy"); (4) every companion adopter's `title`/`subtitle` props read from `BUDDY_PANEL_COPY[<surface>]`, locking surface↔copy alignment (catches "title pulled from a different key than the surface declares"); (5) `/start` does NOT pass `size={88}` (Buddy is the hero, not a calm companion); (6) every BUDDY_PANEL_COPY entry has both `title` and `subtitle` strings. Mirrored as `tests/unit/buddyPanelPlacementContract.test.mjs`. Joined into `scripts/check-contract-routes.sh` so all three architectural-invariant gates (locked routes, v2.7 heading, v2.8 placement) run loudly together as a single pre-test gate.
