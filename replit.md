...
### Learn Detail Pages + Footer Social + Theme Card Linking (Round 4.5)
Surgical, additive A→Z 360° wire-up of three previously-orphaned link surfaces:

1. **`LearnGuides.jsx` & `LearnArticles.jsx` were unclickable cards**: Both pages rendered card-shaped `<div>` and `<article>` elements with `hover:shadow-md` and `cursor-pointer` styling but no actual `<Link>` wrapper, so taps did nothing. Created `client/src/content/learn.js` as a single source of truth holding 6 GUIDES and 6 ARTICLES with `slug`, `body` paragraphs, `relatedHub` (mapped to existing `/hubs/*` routes), and `relatedLabel`. Created `client/src/pages/LearnDetail.jsx` exporting `LearnGuideDetail` (wired to `/learn/guides/:slug`) and `LearnArticleDetail` (wired to `/learn/articles/:slug`) — each renders title + body paragraphs + a "Try it now" CTA card pointing to the related hub + chips linking to sibling guides/articles. Rewrote both listing pages so each card is a wouter `<Link href={...} className=...>` that navigates to the new detail route. Registered all four routes (`/learn/guides`, `/learn/guides/:slug`, `/learn/articles`, `/learn/articles/:slug`) in `App.jsx` via `lazy()`. **Wouter-v3 caveat caught & fixed**: initial implementation used `<Link><a>...</a></Link>`, which produced nested `<a><a>` HTML and broke hydration; corrected to pass `className`/`data-testid`/`aria-label` directly to `<Link>` (wouter v3 renders its own anchor and forwards props).

2. **`SacredFooter.jsx` had FAKE social icons**: Heart/Mail/Sparkles icons linked to `/community`, `/contact`, `/affirmations` (internal nav, not social platforms). Replaced with real icons (Instagram, YouTube, TikTok, X, Facebook) sourced from `client/src/config/social.ts` (`SOCIAL_LINKS`). Used `Music2` from lucide as TikTok stand-in (no native TikTok icon in lucide). Existing `target="_blank" rel="noopener"` already applied to external URLs.

3. **`SacredCard` in `PageTemplate.jsx` ignored `card.href`**: The "Explore by theme" section in `routes.js` (cards: Articles/Guides/Resources/Search) had no href; the SacredCard renderer was a plain `<article>` with no link wrapping. Added `href`/`link`/`path`/`to` support to SacredCard — when present, renders as wouter `<Link>` (internal) or `<a target="_blank" rel="noopener noreferrer">` (external `https?://` URLs). Added hrefs to all four theme cards: Articles → `/learn/articles`, Guides → `/learn/guides`, Resources → `/healing-library`, Search → `/content`. Existing `<article>` rendering preserved when no href present.

Verification: probed all 17 affected routes (3 listings + 12 details + 2 destinations) — all 200. Hydration error confirmed gone (no more nested-anchor warnings in browser console).
