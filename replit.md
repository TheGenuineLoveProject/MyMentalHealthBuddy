...
The legacy `BuddyAvatar` mascot has been replaced with `LumiMascot` from the v2.0 design system in the global Header, Home page (nav badge + hero), and the entire auth flow: Login (`emotion="neutral"`), Register (`emotion="joy"`), ForgotPassword (`emotion="empathy"`), and ResetPassword (`emotion="comfort"`). Other surfaces (Peacescape, CRMPage, CanvaLanding, and various wellness pages) continue to use `BuddyAvatar` until they are migrated, since both renderers coexist additively without conflict.
