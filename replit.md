...
- Boot-time table creation mirrored in `server/db/ensureSchema.mjs` (additive `CREATE TABLE IF NOT EXISTS`); ensureSchema now bootstraps 30 statements (was 22), all green.
- New admin-gated REST surface `server/routes/consciousness.mjs` mounted at `/api/admin/consciousness` via the existing `ADMIN_SUB_ROUTERS` safe-mount block (`adminLimiter → requireAuth → requireAdmin`). Endpoints: `/health`, `/summary`, `/agents` (GET, POST, PATCH), `/decisions` (GET, POST), `/scores` (GET, POST). All write payloads validated; division/status/severity/source enums enforced; kill-switch + lifecycle status mutations recorded.
- New admin panel `client/src/components/admin/ConsciousnessRegistryPanel.jsx` mounted in `CommandCenter.jsx` "Platform Healing & Operations" section. Mirrors SOPMonitorPanel chrome (glass-premium + dual-token fetch). Renders 24h decision/score rollup, division grouping, agent create form, and kill-switch / pause-resume controls. All interactive elements carry `data-testid` and ARIA labels.
...
- Validation: 401 without bearer on all `/api/admin/consciousness/*` endpoints (auth chain enforced); 7/7 admin sub-routers mounted on boot; 3/3 new tables present in Postgres; TypeScript check clean.
- Out of scope this phase (deferred): LangGraph orchestration, NATS messaging, BERT/RoBERTa Layer-2 detector, vector store. Phase 0 is the **persistence + audit foundation** that all future v2.0 capabilities will write to.
