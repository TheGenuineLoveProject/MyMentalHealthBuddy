...
**Round 4 (Apr-27) — PWA cache hygiene** (root cause: deploys looked successful but users saw stale screens because the service worker pre-cached `/index.html` under a hardcoded `glp-cache-v1` name that was never bumped between releases):
- **`client/public/service-worker.js`** rewritten: `CACHE_NAME = \`${CACHE_PREFIX}${BUILD_ID}\`` with `BUILD_ID = "__BUILD_ID__"` template; activate handler now purges every cache whose name starts with `glp-cache-` and is not the current build; HTML/navigation requests use a dedicated `networkFirstHTML` (no caching of HTML responses) so `index.html` is never frozen; `cacheFirst` now restricted to Vite-hashed assets (regex `/\/assets\/.+\.[A-Za-z0-9_-]{6,}\.(...)$/`) which are content-immutable and safe to cache forever.
- **`server/app.mjs`** dynamic SW handler: `app.get(["/service-worker.js", "/serviceWorker.js"], ...)` mounted BEFORE `express.static`/Vite middleware reads the file (dev: `client/public`, prod: `client/dist`), replaces `__BUILD_ID__` with `BUILD_ID` (`process.env.BUILD_ID || REPLIT_DEPLOYMENT_ID || REPL_ID || \`boot-${Date.now()}\``), sets `Cache-Control: no-cache, no-store, must-revalidate`, `Service-Worker-Allowed: /`, and an introspection header `X-MMHB-Build-Id` used by the SOP probe.
- **`spaCacheHeaders`** setHeaders function applied to both `express.static(CLIENT_DIST, ...)` and `express.static(PUBLIC_DIR, ...)`: `index.html` → `no-cache, no-store, must-revalidate`; Vite-hashed assets → `public, max-age=31536000, immutable`; default for everything else.
- **SPA catch-all** (`*` route, dev + prod branches both updated) now sets `Cache-Control: NO_STORE_HEADERS` on the served `index.html` so the SOP `index-html-no-store` probe passes regardless of which branch served the response.
- **SOP Monitor extended to 27 checks** (3 new): (1) `sw-cache-versioned` static — reads `client/public/service-worker.js`, fails if `glp-cache-v1` is hardcoded, passes when `__BUILD_ID__` template + `name.startsWith(CACHE_PREFIX)` purge pattern are both present; (2) `index-html-no-store` — loopback `GET /` and asserts `Cache-Control` contains `no-store`/`no-cache`; (3) `sw-no-store` — loopback `GET /service-worker.js`, asserts (a) no-store header, (b) `__BUILD_ID__` token has been replaced in the body, (c) `X-MMHB-Build-Id` header is present and matches a string in the body. Current SOP state: 27 checks, 21 pass / 6 warn / 0 fail, coverage 78%.
