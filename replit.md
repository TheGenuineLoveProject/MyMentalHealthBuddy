...
- `GET /api/admin/health-deep/metrics` — Prometheus-format SLI scrape endpoint (`text/plain; version=0.0.4`) with `?format=json` variant for structured consumption. Read-only; derives entirely from in-memory ring buffers + the latest persisted probe/watch artifacts. Exposes ~20 `mmhb_*` metrics including `mmhb_health_verdict` (one-hot, resolved from `overallStatus` with totals fallback), `mmhb_health_pass/warn/fail_total`, `mmhb_uptime_seconds`, `mmhb_probe_uptime_percent` (HEALTHY share of probe ring), `mmhb_selfheal_mttr_ms` (mean of `REPAIRED_TO_HEALTHY` durations), `mmhb_selfheal_last_outcome` (one-hot), `mmhb_ai_diagnoses_total`, `mmhb_ai_safety_filtered_total`, `mmhb_ai_prompt_version_info` (label-only build-info pattern), `mmhb_scheduler_{enabled,paused,runs_total,consecutive_fails}`, `mmhb_watch_streak` (window length) + `mmhb_watch_streak_kind{kind="healthy|degraded|needsRepair|internalError"}` breakdown, `mmhb_watch_samples`. All one-hot metrics emit explicit `0` for non-matching labels (compliant with Prometheus convention).

Each AI history entry now records `promptVersion` (exported as `PROMPT_VERSION = "1.0.0"` from `healAI.mjs`) and `safetyFiltered` so a Prometheus scraper / audit can detect prompt-version regressions and how often the destructive-language post-filter activates. `PROMPT_VERSION` should be bumped whenever the system prompt, response schema, or destructive-language filter changes meaningfully.

`client/src/pages/admin/HealthDashboard.jsx` consumes the GET via React Query (60s refetch) and renders the Deep Health panel with totals, per-category breakdown, non-pass items + repair hints, watch streak, **scheduler status pill** (ON/OFF/PAUSED + interval + totalRuns + consecutiveFails + inline pause/resume buttons), **verdict-trend sparkline** (12 colored squares oldest→newest from `probeHistory` with hover tooltips, `role="img"` + aria-label, `motion-reduce:hover:scale-100` gating), **latest AI diagnosis widget** (severity-color pill, optional "Filtered" badge with Shield icon when `safetyFiltered` is true, model + prompt version, summary, next-action callout, collapsible remediation steps with risk-level badges), recent autonomous self-heals, and recent admin re-probes. Three action buttons in the panel header — "Re-probe now" (sage, RefreshCw) + "Self-heal now" (amber, Stethoscope) + "Ask AI" (violet, Sparkles) — are wired via TanStack mutations with toast feedback and **all three mutually disable each other** while any is pending. Uses the `glp-pane` design token.
