...
The Lumi Design System v2.0 is an additive visual system with new tokens, typography, and dark-mode mapping. The Lumi mascot, brand-logo lockup, and Genuine Love Project mandala are rendered from canonical PNG artwork. Components include 8 button variants, 9 link types (including `lumi-link-crisis`), `lumi-card` with hover lift, motion utilities (breathe, float, shimmer, scroll-reveal), and a11y helpers. A `useBuddyEmotion` hook maps context to emotion. A showcase route `/lumi-design-system` demonstrates all tokens, components, and mascot states, including a `/crisis` link and the canonical `SafetyFooter`. All motion respects `prefers-reduced-motion`. Accessibility hardening: `.lumi-btn-love` background was darkened from `hsl(350,70%,62%)` to `hsl(350,72%,42%)` so white-text contrast clears WCAG AA, and a universal `:focus-visible` selector in `lumi-links.css` gives every non-crisis link variant (`-text`, `-nav`, `-footer`, `-inline`, `-brand`, `-external`, `-subtle`, `-ghost`) a consistent amber focus ring with dark-mode color swap, while `.lumi-link-crisis` retains its higher-emphasis red focus indicator.
