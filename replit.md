...
- v5.6 polish — Hero & Top CTA polish (additive). `canva-landing.css` btn-sacred-gold and btn-sacred-secondary refined: gentle `translateY(-1px)` hover (no scale), `translateY(1px)` 90ms press state, soft shadow growth/shrink via 280ms cubic-bezier transitions, palette-tinted focus-visible rings (gold for warm, sage-deep for calm). NEW `.btn-sacred-tertiary` class for V16's "Explore Safely" — sage-deep underline link with hover lift + underline-offset shift + arrow translate. `brand.css` `.btn-premium` / `.btn-secondary-premium` (Pricing tier buttons) gained matching `:active` press states, 3px palette focus rings, hover lift parity, and reduced-motion blankets. Header / Footer skipped (nav-only, no CTA buttons). Tool cards, form/modal buttons, and `/v6` panel untouched per user scope.
