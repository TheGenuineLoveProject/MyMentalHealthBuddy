...
Observability is handled by OpenTelemetry tracing and PagerDuty alerting, integrated as an additive, no-op-safe layer under `server/observability/`. Tracing is loaded via `NODE_OPTIONS='--import ./server/observability/preload.mjs'`. Health probes are filtered from HTTP spans. When `OTEL_EXPORTER_OTLP_ENDPOINT` is unset, all three exporter env vars (`OTEL_TRACES_EXPORTER`, `OTEL_METRICS_EXPORTER`, `OTEL_LOGS_EXPORTER`) are forced to `none` so the SDK never falls back to its default `127.0.0.1:4318` target — preventing the `PeriodicExportingMetricReader` ECONNREFUSED spam in dev/no-collector environments. The PagerDuty alerter logs warnings, dedups locally, dry-runs in non-prod unless forced, and pages only when `PAGERDUTY_ROUTING_KEY` is set in production. Safety-critical call sites use typed wrappers from `server/observability/safetyAlerts.mjs`. Admin-gated diagnostics are available at `/api/admin/observability/`.
...
The legacy `BuddyAvatar` mascot has been replaced with `LumiMascot` from the v2.0 design system on every static-logo / branding surface: the global `Header`, Home page (nav badge + hero), the entire auth flow (Login `neutral`, Register `joy`, ForgotPassword `empathy`, ResetPassword `comfort`), the Dashboard nav badge (`CRMPage` `neutral`), the marketing landing page (`CanvaLanding` header `joy` + footer `neutral`), the global `Footer` (`neutral`), the `SacredFooter` (`neutral`), the parent-brand `TglpNavbar` (`neutral`), and the shared `BrandLogo` primitive (`neutral`). The `PeacescapePage` orphan import was also cleaned up. Three components intentionally still consume `BuddyAvatar` because they are **behavioral / load-bearing** and rely on its dynamic `state` prop (calm/encouraging/curious/celebratory/nudging/crisis) and the documented Buddy v2.11 crisis-color stability contract: `InteractiveBuddy` (the peacescape positive-expression cycle wrapper), `ZenScape` (the immersive zen backdrop with breathing rings + accessory glyph), and `BuddyPanel` (the typed `BuddyState`-driven presentational panel). Both renderers coexist additively without conflict; no behavioral surface was altered.
