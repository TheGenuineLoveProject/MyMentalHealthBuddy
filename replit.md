...
- **Deploy upload size — `.replitignore` is REQUIRED**: The repo accumulates large local-only folders (`backups/` ~2.0 GB, `.local/` ~1.2 GB, `.git/` ~915 MB, `node_modules/` ~411 MB, `attached_assets/` ~99 MB) that without exclusion total ~4.7 GB. Replit's deploy uploader has a per-deploy size/time budget; uploading a multi-GB tarball causes the build job to fail before `npm install` runs, surfacing as a generic "build failed to publish" error with no useful logs. The Apr-26 fix added `.replitignore` (and tightened `.gitignore`) to exclude all bloat folders, dropping the effective upload from 4.7 GB to ~33 MB (client 26M + server 2.4M + public 2.1M + scripts 1.4M + configs). **Whenever you add a new large local-only folder (backups, dumps, fixtures, screenshots, etc.) you must add it to `.replitignore`** or the next deploy will start growing again. Quick verification before any deploy: `du -sh client server shared public scripts content package*.json vite.config.js .replit replit.md 2>/dev/null` should sum to <100 MB; anything dramatically above that means a non-app folder is leaking into the upload set. The `.replitignore` does NOT delete anything from disk — local backups stay where they are; they just aren't shipped to the build container.
