...
Three polish bugs surfaced in user iPad screenshots (IMG_4116, IMG_4120/4121, IMG_4122) were fixed:
- **"Three Steps" cards underline regression** — each step card is wrapped in a wouter `<Link>` that renders as `<a>`, and iOS Safari's UA stylesheet underlined every descendant text node, making the three step cards look like raw hyperlinks. Fix: added `no-underline` Tailwind utility + inline `style={{ textDecoration: 'none' }}` on the `<Link>`, plus belt-and-suspenders CSS in `canva-landing.css`: `.canva-landing .step-card-elite, .canva-landing .step-card-elite *  { text-decoration: none; }` with `text-decoration: none !important` on the card root. Hover/focus still surfaces a sage underline on just the `h3` heading for affordance.
- **CTA body paragraph contrast** — the final CTA section gradient went `var(--glp-sage-deep)` → `var(--glp-sage)` (#062a2a → #1ec890); the body paragraph used Tailwind `text-white/90`, which dropped to ~3.0:1 at the bright sage end (fails AA for normal body text). Tightened the gradient end stop to a deeper mid-sage `#0d4a3d` (still bright enough to feel "alive" against the upper sage-deep but ≥10:1 with white at every stop), and switched the body paragraph color from `text-white/90` to inline `style={{ color: 'var(--glp-paper)' }}` (#fcf6ea → ~14:1 on #062a2a, ~9.4:1 on #0d4a3d).
- **SafetyFooter secondary-nav cramming** — on the homepage footer, the Blog · Newsletter · Disclaimer · Privacy · Crisis Resources strip rendered as one giant word ("BlogNewsletterDisclaimerPrivacyCrisis Resources") despite a `flex flex-wrap gap-4` parent — Tailwind gap was being overridden somewhere in the cascade. Fix: added explicit middle-dot `·` separator spans between each link, replaced the brittle Tailwind utility with a stable `.safety-footer__secondary` class in `canva-landing.css` that uses `display: flex; gap: 0.25rem 0.5rem` plus `display: inline-block; padding: 0 0.35rem` on each `.safety-footer__secondary-link` — so spacing survives even if the gap utility is stripped. Separators are `aria-hidden="true"` and visually subtle (opacity 0.55).
