...
The "Biometric Ingestion Pipeline" (Prompt 3.4) adds opt-in nervous-system telemetry as an additive Consciousness OS layer. Three additive Drizzle tables (`biometric_connections`, `biometric_readings`, `nervous_system_states`) are created via `ensureSchema.mjs`. AES-256-GCM at rest with provider tokens and webhook secrets keyed by HKDF-SHA256 from `JWT_SECRET` (per-purpose info strings). Real OAuth integrations for Oura, Google Fit, and Whoop return `501 NotConfigured` when client credentials are absent — the pipeline never falls back to mock data. Apple HealthKit is supported via an HMAC-signed iOS companion webhook (`POST /api/biometrics/healthkit/webhook`, bypassing CSRF on the path-based allowlist; the global `express.json` parser captures `req.rawBody` so HMAC is computed over exact bytes). Manual self-report is supported via `POST /api/biometrics/upload`. Normalizers map provider-specific schemas to canonical metric types (HRV_RMSSD/SDNN, HEART_RATE_RESTING, SLEEP_TOTAL_MIN, SLEEP_EFFICIENCY_PCT, ACTIVITY_STEPS, ACTIVITY_KCAL, etc.) with per-metric range validation that rejects out-of-physiological-range readings. The `BiometricIngestionService` performs idempotent upsert keyed by `(user_id, device_source, metric_type, recorded_at)` so re-syncs and double-clicks never create duplicates. Nervous-system inference is a transparent z-score heuristic over 7-day vs 30-day baselines (HRV up + HR down → ventral; HRV down + HR up → sympathetic; HRV collapse + HR down + sleep collapse → dorsal); every state response carries a non-clinical educational disclaimer with a `/crisis` link. An opt-in scheduler polls connected providers every 6 hours. Public REST surface at `/api/biometrics`: `GET /meta`, `GET/POST /connections`, `GET /callback/:source`, `POST /sync/:source`, `POST /upload`, `POST /healthkit/webhook` (HMAC-only), `GET /data`, `GET /latest`, `GET /state`, `POST /state/compute`, `DELETE /disconnect/:source`. Per-route 60/min IP limiter; cross-user isolation enforced by `requireAuth` ownership checks on every reading row.
