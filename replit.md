...
The "Buddy Engine" is a separate visual companion layer that renders alongside the AI Chat orchestrator, with its own typed contract (`/api/buddy` v1.9) for visual output and DOM mirroring for accessibility and testing. The visual avatar (BuddyAvatar v1.13 visible-presence redesign) uses a layered SVG design with real dimensional depth — a soft state-tinted ambient aura behind the whole figure (opacity boosted to 0.55 so it actually reads on light page surfaces), a state-tinted floor-glow pool beneath Buddy's feet for a gentle floating feel (opacity 0.65), tinted radial-gradient head/body/face shells with 2px rim strokes that define a clean silhouette against any background, a wide bridging neck collar plus body-to-head overlap that forms natural shoulders fusing head and body into one silhouette, a multi-ring (3-ring) halo'd heart with top-left highlight + brighter core, a state-tinted chest status LED beneath the heart, two subtle plate seam lines on the body for character, animated highlight specks + crescent shadows + outer halo rings on the eyes for round-eye depth, brighter cheek blush dots, an antenna with a base mount, double-stroke stem, inner glow ring, and softly pulsing larger outer halo tip, rounded hand caps on the arms with highlights, a head-dome highlight crescent for 3D-sphere read, and a glassy face-screen sheen — plus a layered CSS drop-shadow filter (3px contact + 10px cast) on the SVG that makes Buddy visibly "float" off the page on both light and dark surfaces. All rendered within the same 200×240 viewBox and using only the three canonical CSS variables (`--buddy-eye-color`, `--buddy-heart-color`, `--buddy-heart-pulse`) so every accent shifts color naturally with the BuddyState. State-driven motion variants include a compassionate head-tilt for the encouraged state and CSS-only floating sparkle particles for the celebrate state (both fully gated by `prefers-reduced-motion`). The hero on `/start` renders Buddy at 180×180 to give the polish real presence. It manages placement rules for different UI surfaces and incorporates robust invariant scripts to ensure architectural contracts related to heading semantics, placement geometry, telemetry parity, DOM-mirror parity, crisis-color stability, strict-protected-file import boundary, and VISUAL_MAP exhaustiveness + state↔key consistency + free-text fallback coverage. These scripts act as pre-test gates (joined via `scripts/check-contract-routes.sh`) to enforce design, safety, and separation contracts; the import-boundary guard locks the architectural separation between Buddy Engine source files and the strict-protected `/api/ai/chat` handlers, the orchestrator/provider/memory/profile/summary/crisis logic in `server/ai/`, and the `/start` page internals; the VISUAL_MAP guard locks BUDDY_STATES↔VISUAL_MAP exhaustiveness, per-entry key↔state consistency, all 8 BuddyVisualOutput required fields with structurally-valid values, the crisis safety-routing flag (`safetyMode: "crisis_safe"` exclusive to the `crisis` state), and `resolveBuddyState` regex-fallback coverage for every non-default BuddyState.
