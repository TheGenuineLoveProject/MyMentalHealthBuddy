...
For *continuous* observation, `scripts/heal-watch.mjs` runs heal-360 on a configurable interval (default 300s, override with `--interval=<sec>`, single-shot via `--once`). It writes a rolling status file to `docs/health-watch-status.json` containing the latest sample, a `streak` window over the last 10 samples (healthy/degraded/needsRepair counts), and up to `HISTORY_LIMIT` (50) historical samples. Loop mode handles SIGINT/SIGTERM gracefully; single-shot mode exits with the same 0/1/2/3 verdict codes as heal-360. Read-only — never repairs.

The deep-health surface is wired into the Command Center: `GET /api/admin/health-deep` (admin-only, served from `server/routes/admin.mjs`) reads both files server-side and returns `{ verdict, totals, categories, nonPass[], watch{ latest, streak } }` without re-running probes. The existing `client/src/pages/admin/HealthDashboard.jsx` consumes this endpoint via React Query (60s refetch) and renders a new "Deep Health (heal-360)" panel with PASS/WARN/FAIL totals, per-category breakdown, an itemized non-pass list with repair hints, and the recent watch streak.
...
The shared `WellnessPageShell` (used by `/journal`, `/mood`, `/state`, `/chat`, `/ai-chat`, and also `/dashboard`, `/pricing`) carries a `wellness-shell` root class that activates `client/src/styles/wellness-shell.css`. The stylesheet adds: a subtle gold/sage radial backdrop fixed behind the page (z-index −1), refined sanctuary depth on the main content section (gradient bg, animated tri-color top accent bar, hover lift), refined quicknav pill hover (sage tint + soft elevation), and a serif gradient on the page title. Effects are scoped to `.wellness-shell` (no leak to other surfaces) and fully gated by `prefers-reduced-motion`.

### Peace Scape — Sanctuary Depth
The `/peacescape` surface (which uses its own `<ZenScape>` hero rather than `WellnessPageShell`) carries a `peacescape-shell` root class on `PeacescapePage.jsx` that activates `client/src/styles/peacescape-shell.css`. The stylesheet adds: a fixed dual-quadrant gold + sage radial backdrop (z-index −1), sanctuary depth on every roadmap-phase card (`[data-testid^='card-roadmap-phase-']`) with an animated top accent bar and hover lift, a permanent gold ring on the "Phase 1 · live now" card, and refined sanctuary chrome on the bottom callouts (`[data-testid='callout-customizer']` and `[data-testid='callout-companion']`) with a vertical tri-color accent bar on the leading edge. Section heading also gets the same serif sage-deep gradient as the wellness shell. Fully scoped to `.peacescape-shell` and gated by `prefers-reduced-motion`.
