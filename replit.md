...
The **Admin Command Center** is a fully-functional operations console. The flow is: visit `/admin-login` (or `/admin/login`) → paste the `ADMIN_TOKEN` env secret → POST `/api/admin/verify-token` → server returns a 4-hour admin JWT with `{role: "admin"}` → client stores it in `sessionStorage["adminVerified"]` + `sessionStorage["adminSessionToken"]` → redirect to `/admin` (rendered by `client/src/pages/admin/CommandCenter.jsx`, gated by `<AdminGuard>`). The full admin route surface (30+ pages including CommandCenter, NewsletterAdmin, SocialDashboard, HealthDashboard, AuditLogExplorer, SecurityDashboard, AdminUsers, RevenueAdmin, AdminTools) is mounted under `/api/admin/*` with the strict chain `adminLimiter → requireAuth → requireAdmin` from `server/middleware/auth.mjs`. To resolve the chicken-and-egg problem (the verify-token endpoint cannot itself require admin auth), `app.post("/api/admin/verify-token", adminLimiter, ...)` is mounted **before** the protected `/api/admin` chain in `server/app.mjs` and re-dispatches into `adminRoutes` so the original handler logic stays in `server/routes/admin.mjs`. CSRF middleware (`server/security/csrf.mjs`) skips this endpoint plus the public newsletter / leads / contact / feedback POST forms. The admin handler uses ESM-imported `crypto` (`import crypto from "node:crypto"`) for timing-safe token comparison.

The application bootstraps its database schema at boot via `server/db/ensureSchema.mjs`, which runs idempotent `CREATE TABLE IF NOT EXISTS` statements (matching `shared/schema.mjs` exactly) for `journals`, `moods`, `gratitude_entries`, `blog_posts`, `newsletter_subscribers`, `leads`, and `analytics_events`. This is necessary because `package.json` does not define a `db:push` script and `drizzle.config.ts` points to an empty `database/schema/` directory, so `drizzle-kit push` is a no-op against this codebase. `ensureSchema()` is awaited inside `server/app.mjs` immediately before `app.listen(...)`. The `users` table is additionally bootstrapped on-demand by `auth.mjs::ensureUsersTable()` (legacy double-safety). Together these guarantee that blog, newsletter, leads, growth-journey, and auth flows return 200 from the first request, even on a brand-new database.
