...
export function GlowEffect({ color = 'serenity', customColor, intensity = 'moderate', pulse = false, children, className = '', }) {
 ...
 */
export function AmbientLightSpots({ scene = 'serenity', count = 3, }) {
    const reducedMotion = useReducedMotion();
    const lightColors = {
    ...
    return (_jsx("div", { className: "absolute inset-0 overflow-hidden pointer-events-none", "aria-hidden": "true", children: lights.map((light) => (_jsx("div", { className: "absolute rounded-full", style: {
                width: `${light.size}px`,
